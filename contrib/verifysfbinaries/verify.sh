...
###   This script attempts to download the signature file SHA256SUMS.asc from presidentielcoin.org
...
WORKINGDIR="/tmp/presidentielcoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://presidentielcoin.org/bin/"
VERSIONPREFIX="presidentielcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'presidentielcoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Presidentielcoin Core binary release signing key installed?"
   fi
