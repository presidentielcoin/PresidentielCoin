...
The RPM spec file provided here is for Presidentielcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://presidentielcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Presidentielcoin git repository](https://github.com/presidentielcoin/presidentielcoin2) but are not part of
...
Currently only `Source10` is used, for the example `presidentielcoin.conf` file.
...
in the presidentielcoin git repository. At present only an SVG version of the Presidentielcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the presidentielcoin
...
needed when 0.12.1 is released, a proper fix is already in the Presidentielcoin git
master branch.
...
idea to build Presidentielcoin Core against a static version of that library compiled
...
The `presidentielcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `presidentielcoin-server` package. At this point, most RPM based
...
The `presidentielcoin-server` package should have SELinux support. How to properly do
...
Presidentielcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/presidentielcoin/). He has been
packaging Presidentielcoin for Fedora at least since 2012.
...
4. I renamed the `presidentielcoin` package that contains the Qt GUI to `presidentielcoin-core` as
that appears to be how the general population refers to it, in contrast to
`presidentielcoin-xt` or `presidentielcoin-classic`. I wanted to make sure the general population
