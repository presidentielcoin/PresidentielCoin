...
bool ValidatePaymentOptionsString(const std::string &paymentOptionsString) {
	...
	for (size_t i = 0; i < strs.size(); i++) {
		if(strs[i].compare("BTC") != 0 && strs[i].compare("PRC") != 0 && strs[i].compare("ZEC") != 0) {
			retval = false;
			break;
		}
	}
...
bool GetTxOfOfferAccept(const vector<unsigned char> &vchOffer, const vector<unsigned char> &vchOfferAccept,
	...
	if (!GetPresidentielcoinTransaction(acceptOffer.nHeight, acceptOffer.txHash, tx, Params().GetConsensus()))
		return false;
...
bool CheckOfferInputs(const CTransaction &tx, int op, int nOut, const vector<vector<unsigned char> > &vvchArgs, const CCoinsViewCache &inputs, bool fJustCheck, int nHeight, string &errorMessage, bool dontaddtodb) {
	...
	int nDataOut;
	if(!GetPresidentielcoinData(tx, vchData, vchHash, nDataOut) || !theOffer.UnserializeFromData(vchData, vchHash))
	{
		errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR ERRCODE: 1000 - " + _("Cannot unserialize data inside of this transaction relating to an offer");
		return true;
	}
	// Make sure offer outputs are not spent by a regular transaction, or the offer would be lost
	if (tx.nVersion != PRESIDENTIELCOIN_TX_VERSION)
	{
		errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1001 - " + _("Non-Presidentielcoin transaction found");
		return true;
	}
	if(fJustCheck)
	{
		...
		{
			errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1002 - " + _("Offer arguments incorrect size");
			return error(errorMessage.c_str());
		}
		else if(op == OP_OFFER_ACCEPT && vvchArgs.size() != 4)
		{
			errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1003 - " + _("OfferAccept arguments incorrect size");
			return error(errorMessage.c_str());
		}
		...
		if(op == OP_OFFER_ACCEPT)
		{
			...
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1004 - " + _("Hash provided doesn't match the calculated hash of the data");
				return true;
			}
		}
		else
		{
			...
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1005 - " + _("Hash provided doesn't match the calculated hash of the data");
				return true;
			}
		}
	...
	if(fJustCheck)
	{
		...
		{
			errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1006 - " + _("Offer guid too long");
			return error(errorMessage.c_str());
		}

		if(theOffer.sDescription.size() > MAX_VALUE_LENGTH)
		{
			errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1007 - " + _("Offer description too long");
			return error(errorMessage.c_str());
		}
		if(theOffer.sTitle.size() > MAX_NAME_LENGTH)
		{
			errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1008 - " + _("Offer title too long");
			return error(errorMessage.c_str());
		}
		if(theOffer.sCategory.size() > MAX_NAME_LENGTH)
		{
			errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1009 - " + _("Offer category too long");
			return error(errorMessage.c_str());
		}
		if(theOffer.vchLinkOffer.size() > MAX_GUID_LENGTH)
		{
			errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1010 - " + _("Offer link guid hash too long");
			return error(errorMessage.c_str());
		}
		if(theOffer.sCurrencyCode.size() > MAX_GUID_LENGTH)
		{
			errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1011 - " + _("Offer curreny too long");
			return error(errorMessage.c_str());
		}
		if(theOffer.vchGeoLocation.size() > MAX_NAME_LENGTH)
		{
			errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1012 - " + _("Offer geolocation too long");
			return error(errorMessage.c_str());
		}
		if(theOffer.linkWhitelist.entries.size() > 1)
		{
			errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1013 - " + _("Offer has too many affiliate entries, only one allowed per transaction");
			return error(errorMessage.c_str());
		}
		if(!theOffer.vchOffer.empty() && theOffer.vchOffer != vvchArgs[0])
		{
			errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1014 - " + _("Offer guid in the data output does not match the guid in the transaction");
			return error(errorMessage.c_str());
		}
		switch (op) {
		case OP_OFFER_ACTIVATE:
			...
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1015 - " + _("Alias input mismatch");
				return error(errorMessage.c_str());
			}
			if(!ValidatePaymentOptionsMask(theOffer.paymentOptions))
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1016 - " + _("Invalid payment option");
				return error(errorMessage.c_str());
			}
			if(!theOffer.accept.IsNull())
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1017 - " + _("Cannot have accept information on offer activation");
				return error(errorMessage.c_str());
			}
			if ( theOffer.vchOffer != vvchArgs[0])
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1018 - " + _("Offer input and offer guid mismatch");
				return error(errorMessage.c_str());
			}
			if(!theOffer.vchLinkOffer.empty())
			{
				...
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1019 - " + _("Commission must between -90 and 100");
					return error(errorMessage.c_str());
				}
			}
			else
			{
				...
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1020 - " + _("Offer category cannot be empty");
					return error(errorMessage.c_str());
				}
				if(theOffer.sTitle.size() < 1)
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1021 - " + _("Offer title cannot be empty");
					return error(errorMessage.c_str());
				}
				if(theOffer.paymentOptions <= 0)
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1022 - " + _("Invalid payment option specified");
					return error(errorMessage.c_str());
				}
			}
			if(theOffer.nQty < -1)
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1023 - " + _("Quantity must be greater than or equal to -1");
				return error(errorMessage.c_str());
			}
			if(!theOffer.vchCert.empty() && theOffer.nQty != 1)
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1024 - " + _("Quantity must be 1 for a digital offer");
				return error(errorMessage.c_str());
			}
			if(theOffer.nPrice <= 0)
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1025 - " + _("Offer price must be greater than 0");
				return error(errorMessage.c_str());
			}
		...
		case OP_OFFER_UPDATE:
			if(!IsAliasOp(prevAliasOp) || vvchPrevAliasArgs.empty() || theOffer.vchAlias != vvchPrevAliasArgs[0])
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1026 - " + _("Alias input mismatch");
				return error(errorMessage.c_str());
			}
			...
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1026 - " + _("Alias link input mismatch");
				return error(errorMessage.c_str());
			}			
			if(theOffer.paymentOptions > 0 && !ValidatePaymentOptionsMask(theOffer.paymentOptions))
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1027 - " + _("Invalid payment option");
				return error(errorMessage.c_str());
			}
			if(!theOffer.accept.IsNull())
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1028 - " + _("Cannot have accept information on offer update");
				return error(errorMessage.c_str());
			}
			if ( theOffer.vchOffer != vvchArgs[0])
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1029 - " + _("Offer input and offer guid mismatch");
				return error(errorMessage.c_str());
			}
			...
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1030 - " + _("Discount must be between 0 and 99");
				return error(errorMessage.c_str());
			}

			if(theOffer.nQty < -1)
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1031 - " + _("Quantity must be greater than or equal to -1");
				return error(errorMessage.c_str());
			}
			if(!theOffer.vchCert.empty() && theOffer.nQty != 1)
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1032 - " + _("Quantity must be 1 for a digital offer");
				return error(errorMessage.c_str());
			}
			if(theOffer.nPrice <= 0)
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1033 - " + _("Offer price must be greater than 0");
				return error(errorMessage.c_str());
			}
			if(theOffer.nCommission > 100 || theOffer.nCommission < -90)
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1034 - " + _("Commission must between -90 and 100");
				return error(errorMessage.c_str());
			}
			break;
		case OP_OFFER_ACCEPT:
			...
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1035 - " + _("Alias input mismatch");
				return error(errorMessage.c_str());
			}
			if(!theOfferAccept.feedback.empty())
			{
				...
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1036 - " + _("Invalid feedback transaction");
					return error(errorMessage.c_str());
				}
				if(theOfferAccept.feedback[0].vchFeedback.empty())
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1037 - " + _("Cannot leave empty feedback");
					return error(errorMessage.c_str());
				}
				if(theOfferAccept.feedback[0].vchFeedback.size() > MAX_NAME_LENGTH)
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1038 - " + _("Feedback too long");
					return error(errorMessage.c_str());
				}
				if(theOfferAccept.feedback.size() > 1)
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1039 - " + _("Cannot only leave one feedback per transaction");
					return error(errorMessage.c_str());
				}
				break;
			}
			else
			{
                ...
                {
                    errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1040 - " + _("Invalid payment option");
                    return error(errorMessage.c_str());
                }
			}
			...
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1041 - " + _("Offeraccept object cannot be empty");
				return error(errorMessage.c_str());
			}
			if (!IsValidAliasName(theOfferAccept.vchBuyerAlias))
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1042 - " + _("Invalid offer buyer alias");
				return error(errorMessage.c_str());
			}
			if (vvchArgs.size() <= 1 || vvchArgs[1].size() > MAX_GUID_LENGTH)
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1043 - " + _("Offeraccept transaction with guid too big");
				return error(errorMessage.c_str());
			}
			if (theOfferAccept.vchAcceptRand.size() > MAX_GUID_LENGTH)
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1044 - " + _("Offer accept hex guid too long");
				return error(errorMessage.c_str());
			}
			if (theOfferAccept.vchMessage.size() > MAX_ENCRYPTED_NAME_LENGTH)
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1045 - " + _("Payment message too long");
				return error(errorMessage.c_str());
			}
			if (theOffer.vchOffer != vvchArgs[0])
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1046 - " + _("Offer input and offer guid mismatch");
				return error(errorMessage.c_str());
			}
		...
		default:
			errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1047 - " + _("Offer transaction has unknown op");
			return error(errorMessage.c_str());
		}
	}
	...
	if (!fJustCheck ) {
		...
		if(op != OP_OFFER_ACTIVATE) {
			...
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1048 - " + _("Failed to read from offer DB");
				return true;
			}
			if(serializedOffer.vchAlias != theOffer.vchAlias)
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1049 - " + _("Offer alias mismatch");
		...
		else if(op == OP_OFFER_ACTIVATE)
		{
			...
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1050 - " + _("Offer already exists");
				return true;
			}
			...
			if(!theOffer.vchLinkOffer.empty())
			{
				...
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1051 - " + _("Linked offer not found. It may be expired");
				}
				else
				{
					...
					{
						errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1052 - " + _("Failed to write to offer link to DB");
						return error(errorMessage.c_str());
					}
				}
			}
			...
			if(theOfferAccept.bPaymentAck)
			{
				...
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1053 - " + _("Could not find offer accept from mempool or disk");
					return true;
				}
				if(!acceptOffer.vchLinkOffer.empty())
				{
					...
					{
						errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1054 - " + _("Could not get linked offer");
						return true;
					}
					if(theOfferAccept.vchBuyerAlias != linkOffer.vchAlias)
					{
						errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1055 - " + _("Only root merchant can acknowledge offer payment");
						return true;
					}
				}	
				else
				{
					...
					{
						errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1056 - " + _("Only merchant can acknowledge offer payment");
						return true;
					}
				}
				if(offerAccept.bPaymentAck)
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1057 - " + _("Offer payment already acknowledged");
				}
				...
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1058 - " + _("Failed to write to offer DB");
					return error(errorMessage.c_str());
				}
				if (fDebug)
			...
			else if(!theOfferAccept.feedback.empty())
			{
				if (!GetOfferAccept(vvchArgs[0], vvchArgs[1], acceptOffer, offerAccept))
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1059 - " + _("Could not find offer accept from mempool or disk");
					return true;
				}
				...
				if(theOfferAccept.feedback[0].nFeedbackUserFrom == FEEDBACKBUYER)
				{
					...
					{
						errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1060 - " + _("Only buyer can leave the seller feedback");
						return true;
					}
				}
				else if(theOfferAccept.feedback[0].nFeedbackUserFrom == FEEDBACKSELLER)
				{
					...
					{
						errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1061 - " + _("Only seller can leave the buyer feedback");
						return true;
					}
				}
				else
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1062 - " + _("Unknown feedback user type");
					return true;
				}
				...
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1063 - " + _("Cannot exceed 10 buyer feedbacks");
					return true;
				}
				else if(feedbackSellerCount >= 10 && theOfferAccept.feedback[0].nFeedbackUserFrom == FEEDBACKSELLER)
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1064 - " + _("Cannot exceed 10 seller feedbacks");
					return true;
				}
			...
			if (GetOfferAccept(vvchArgs[0], vvchArgs[1], acceptOffer, offerAccept))
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1065 - " + _("Offer payment already exists");
				return true;
			}
			...
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1066 - " + _("Cannot purchase this private offer, must purchase through an affiliate");
				return true;
			}
			if(!myPriceOffer.vchLinkOffer.empty())
			{
				...
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1067 - " + _("Could not get linked offer");
					return true;
				}
				...
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1068 - " + _("Cannot find alias for this linked offer. It may be expired");
					return true;
				}
				if(!IsPaymentOptionInMask(linkOffer.paymentOptions, theOfferAccept.nPaymentOption))
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1069 - " + _("User selected payment option not found in list of accepted offer payment options");
					return true;
				}
				...
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1070 - " + _("External chain payment cannot be made with this offer");
					return true;
				}
				linkOffer.linkWhitelist.GetLinkEntryByHash(theOffer.vchAlias, entry);
				if(entry.IsNull())
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1071 - " + _("Linked offer alias does not exist on the root offer affiliate list");
					return true;
				}
				if(theOffer.nCommission <= -entry.nDiscountPct)
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1072 - " + _("This resold offer must be of higher price than the original offer including any discount");
					return true;
				}
			}
			if(!IsPaymentOptionInMask(myPriceOffer.paymentOptions, theOfferAccept.nPaymentOption))
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1073 - " + _("User selected payment option not found in list of accepted offer payment options");
				return true;
			}
			...
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1074 - " + _("External chain payment cannot be made with this offer");
				return true;
			}		
			if(theOfferAccept.txExtId.IsNull() && theOfferAccept.nPaymentOption != PAYMENTOPTION_SYS)
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1075 - " + _("External chain payment txid missing");
				return true;
			}
			if(myPriceOffer.sCategory.size() > 0 && boost::algorithm::starts_with(stringFromVch(myPriceOffer.sCategory), "wanted"))
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1076 - " + _("Cannot purchase a wanted offer");
				return true;
			}
			vector<CAliasIndex> vtxAlias;
			bool isExpired = false;
			...
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1077 - " + _("Cannot find alias for this offer. It may be expired");
				return true;
			}
			// check that user pays enough in presidentielcoin if the currency of the offer is not external purchase
			if(theOfferAccept.txExtId.IsNull())
			{
				...
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1078 - " + _("Offer payment does not specify the correct payment amount");
					return true;
				}
				...
				if(nOutPayment < 0)
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1079 - " + _("Offer payment does not pay enough according to the offer price");
					return true;
				}
				if(!myPriceOffer.vchLinkOffer.empty())
				{
					...
					if(nOutCommission < 0)
					{
						errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1080 - " + _("Offer payment does not include enough commission to affiliate");
						return true;
					}
					CPresidentielcoinAddress destaddy;
					if (!ExtractDestination(tx.vout[nOutPayment].scriptPubKey, payDest))
					{
						errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1081 - " + _("Could not extract payment destination from scriptPubKey");
						return true;
					}
					destaddy = CPresidentielcoinAddress(payDest);
					CPresidentielcoinAddress commissionaddy;
					if (!ExtractDestination(tx.vout[nOutCommission].scriptPubKey, commissionDest))
					{
						errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1082 - " + _("Could not extract commission destination from scriptPubKey");
						return true;
					}
					commissionaddy = CPresidentielcoinAddress(commissionDest);
					CPresidentielcoinAddress aliaslinkaddy;
					GetAddress(linkAlias, &aliaslinkaddy);
					if(aliaslinkaddy.ToString() != destaddy.ToString())
					{
						errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1083 - " + _("Payment destination does not match merchant address");
						return true;
					}
					CPresidentielcoinAddress aliasaddy;
					GetAddress(alias, &aliasaddy);
					if(aliasaddy.ToString() != commissionaddy.ToString())
					{
						errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1084 - " + _("Commission destination does not match affiliate address");
						return true;
					}
				}
				else
				{
					...
					if (!ExtractDestination(tx.vout[nOutPayment].scriptPubKey, payDest))
					{
						errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1085 - " + _("Could not extract payment destination from scriptPubKey");
						return true;
					}
					CPresidentielcoinAddress destaddy(payDest);
					CPresidentielcoinAddress aliasaddy;
					GetAddress(alias, &aliasaddy);
					if(aliasaddy.ToString() != destaddy.ToString())
					{
						errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1086 - " + _("Payment destination does not match merchant address");
						return true;
					}
				}
			}
			...
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1087 - " + _("Cannot extract destination from output script");
				return true;
			}
			CPresidentielcoinAddress destaddy(dest);
			CPresidentielcoinAddress aliasaddy;
			GetAddress(alias, &aliasaddy);
			if(aliasaddy.ToString() != destaddy.ToString())
			{
				errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1088 - " + _("Payment address does not match merchant address");
				return true;
			}
			...
			if(nQty != -1)
			{
				...
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1089 - " + _("Not enough quantity left in this offer for this purchase");
					return true;
				}
			...
			if (!linkOffer.IsNull())
			{
				...
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1090 - " + _("Failed to write to offer link to DB");
					return error(errorMessage.c_str());
				}
			}
			if(!theOfferAccept.txExtId.IsNull())
			{
				...
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1091 - " + _("BTC Transaction ID specified was already used to pay for an offer");
					return true;
				}
				if(!dontaddtodb && !pofferdb->WriteOfferTx(theOffer.vchOffer, theOfferAccept.txExtId))
				{
					errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1092 - " + _("Failed to BTC Transaction ID to DB");
					return error(errorMessage.c_str());
				}
			}
		...
		if(op == OP_OFFER_UPDATE)
		{
			...
			if(serializedOffer.linkWhitelist.entries.size() == 1)
			{
				...
				if(serializedOffer.linkWhitelist.entries[0].nDiscountPct == 127)
				{
					...
					{
						errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1093 - " + _("Whitelist is already empty");
					}
					else
				...
				else
				{
					...
					{
						errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1094 -" + _("Too many affiliates for this offer");
					}
					else if(!serializedOffer.linkWhitelist.entries[0].aliasLinkVchRand.empty())
					{
						...
						{
							errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1095 - " + _("Cannot find the alias you are trying to offer affiliate list. It may be expired");
						}
					}
				}
		...
		if (!dontaddtodb && !pofferdb->WriteOffer(vvchArgs[0], vtxPos))
		{
			errorMessage = "PRESIDENTIELCOIN_OFFER_CONSENSUS_ERROR: ERRCODE: 1096 - " + _("Failed to write to offer DB");
			return error(errorMessage.c_str());
		}
...
UniValue offernew(const UniValue& params, bool fHelp) {
	if (fHelp || params.size() < 7 || params.size() > 12)
		throw runtime_error(
		"offernew <alias> <category> <title> <quantity> <price> <description> <currency> [cert. guid] [payment options=PRC] [geolocation=''] [safe search=Yes] [private='0']\n"
						...
						"<paymentOptions> 'PRC' to accept PRC only, 'BTC' for BTC only, 'ZEC' for zcash only, or a |-delimited string to accept multiple currencies (e.g. 'BTC|PRC' to accept BTC or PRC). Leave empty for default. Defaults to 'PRC'.\n"
	...
	if (!GetTxOfAlias(vchAlias, alias, aliastx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1500 - " + _("Could not find an alias with this name"));
    if(!IsMyAlias(alias)) {
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1501 - " + _("This alias is not yours"));
    }
	COutPoint outPoint;
	int numResults  = aliasunspent(vchAlias, outPoint);	
	wtxAliasIn = pwalletMain->GetWalletTx(outPoint.hash);
	if (wtxAliasIn == NULL)
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1502 - " + _("This alias is not in your wallet"));
	...
	try {
		...
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1503 - " + _("Invalid quantity value, must be less than 4294967296 and greater than or equal to -1"));
	}
	...
	if(!ValidatePaymentOptionsString(paymentOptions))
	{
		...
		string err = "PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1504 - " + _("Could not validate the payment options value");
	...
	int precision = 2;
	CAmount nPricePerUnit = convertCurrencyCodeToPresidentielcoin(alias.vchAliasPeg, vchCurrency, fPrice, chainActive.Tip()->nHeight, precision);
	if(nPricePerUnit == 0)
	{
		string err = "PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1505 - " + _("Could not find currency in the peg alias");
		throw runtime_error(err.c_str());
	}
	...
	if(!vchCert.empty())
	{
		...
		{
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1506 - " + _("Creating an offer with a cert that does not exist"));
		}
		else if(!boost::algorithm::starts_with(stringFromVch(vchCat), "certificates"))
		{
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1507 - " + _("Offer selling a certificate must use a certificate category"));
		}
		else if(theCert.vchAlias != vchAlias)
		{
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1508 - " + _("Cannot create this offer because the certificate alias does not match the offer alias"));
		}
	}
	else if(boost::algorithm::starts_with(stringFromVch(vchCat), "certificates"))
	{
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1509 - " + _("Offer not selling a certificate cannot use a certificate category"));
	}
	// this is a presidentielcoin transaction
	CWalletTx wtx;

	// generate rand identifier
	vector<unsigned char> vchOffer = vchFromString(GeneratePresidentielcoinGuid());
	...
	CPresidentielcoinAddress aliasAddress;
	...
	SendMoneyPresidentielcoin(vecSend, recipient.nAmount+fee.nAmount+aliasRecipient.nAmount, false, wtx, wtxAliasIn, outPoint.n, alias.multiSigInfo.vchAliases.size() > 0);
...
UniValue offerlink(const UniValue& params, bool fHelp) {
	...
	const CWalletTx *wtxAliasIn = NULL;
	if (!GetTxOfAlias(vchAlias, alias, aliastx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1510 - " + _("Could not find an alias with this name"));
    if(!IsMyAlias(alias)) {
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1511 - " + _("This alias is not yours"));
    }
	COutPoint outPoint;
	int numResults  = aliasunspent(vchAlias, outPoint);	
	wtxAliasIn = pwalletMain->GetWalletTx(outPoint.hash);
	if (wtxAliasIn == NULL)
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1512 - " + _("This alias is not in your wallet"));
	...
	if (vchLinkOffer.empty() || !GetTxOfOffer( vchLinkOffer, linkOffer, tx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1513 - " + _("Could not find an offer with this guid"));
	...
	if(linkOffer.linkWhitelist.GetLinkEntryByHash(vchAlias, entry))
	{
		...
		{
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1514 - " + _("This resold offer must be of higher price than the original offer including any discount"));
		}
	}
	// make sure alias exists in the root offer affiliate list
	else
	{
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1515 - " + _("Cannot find this alias in the parent offer affiliate list"));
	}
	if (!linkOffer.vchLinkOffer.empty())
	{
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1516 - " + _("Cannot link to an offer that is already linked to another offer"));
	}
	else if(linkOffer.sCategory.size() > 0 && boost::algorithm::starts_with(stringFromVch(linkOffer.sCategory), "wanted"))
	{
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1517 - " + _("Cannot link to a wanted offer"));
	}
...
UniValue offeraddwhitelist(const UniValue& params, bool fHelp) {
	...
	// this is a presidentielcoin txn
	CScript scriptPubKeyOrig;
	...
	if (!GetTxOfOffer( vchOffer, theOffer, tx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1518 - " + _("Could not find an offer with this guid"));

	CTransaction aliastx;
	CAliasIndex theAlias;
	const CWalletTx *wtxAliasIn = NULL;
	if (!GetTxOfAlias( theOffer.vchAlias, theAlias, aliastx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1519 - " + _("Could not find an alias with this name"));

	if(!IsMyAlias(theAlias)) {
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1520 - " + _("This alias is not yours"));
	}
	COutPoint outPoint;
	int numResults  = aliasunspent(theOffer.vchAlias, outPoint);	
	wtxAliasIn = pwalletMain->GetWalletTx(outPoint.hash);
	if (wtxAliasIn == NULL)
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1521 - " + _("This alias is not in your wallet"));
	...
	if(theOffer.linkWhitelist.GetLinkEntryByHash(vchAlias, foundEntry))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1522 - " + _("This alias entry already exists on affiliate list"));
	...
	SendMoneyPresidentielcoin(vecSend, recipient.nAmount+fee.nAmount+aliasRecipient.nAmount, false, wtx, wtxAliasIn, outPoint.n, theAlias.multiSigInfo.vchAliases.size() > 0);
...
UniValue offerremovewhitelist(const UniValue& params, bool fHelp) {
	...
	const CWalletTx *wtxAliasIn = NULL;
	if (!GetTxOfOffer( vchOffer, theOffer, tx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1523 - " + _("Could not find an offer with this guid"));
	CTransaction aliastx;
	CAliasIndex theAlias;
	if (!GetTxOfAlias( theOffer.vchAlias, theAlias, aliastx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1524 - " + _("Could not find an alias with this name"));
	if(!IsMyAlias(theAlias)) {
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1525 - " + _("This alias is not yours"));
	}
	COutPoint outPoint;
	int numResults  = aliasunspent(theOffer.vchAlias, outPoint);	
	wtxAliasIn = pwalletMain->GetWalletTx(outPoint.hash);
	if (wtxAliasIn == NULL)
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1526 - " + _("This alias is not in your wallet"));
	...
	COfferLinkWhitelistEntry foundEntry;
	if(!theOffer.linkWhitelist.GetLinkEntryByHash(vchAlias, foundEntry))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1527 - " + _("This alias entry was not found on affiliate list"));
	theOffer.ClearOffer();
	theOffer.nHeight = chainActive.Tip()->nHeight;
...
UniValue offerclearwhitelist(const UniValue& params, bool fHelp) {
	...
	// this is a presidentielcoind txn
	CWalletTx wtx;
	CScript scriptPubKeyOrig;
	...
	const CWalletTx *wtxAliasIn = NULL;
	if (!GetTxOfOffer( vchOffer, theOffer, tx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1528 - " + _("Could not find an offer with this guid"));
	CTransaction aliastx;
	CAliasIndex theAlias;
	if (!GetTxOfAlias(theOffer.vchAlias, theAlias, aliastx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1529 - " + _("Could not find an alias with this name"));

	if(!IsMyAlias(theAlias)) {
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1530 - " + _("This alias is not yours"));
	}
	COutPoint outPoint;
	int numResults  = aliasunspent(theOffer.vchAlias, outPoint);	
	wtxAliasIn = pwalletMain->GetWalletTx(outPoint.hash);
	if (wtxAliasIn == NULL)
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1531 - " + _("This alias is not in your wallet"));
...
UniValue offerupdate(const UniValue& params, bool fHelp) {
	...
	if(!ValidatePaymentOptionsString(paymentOptions))
	{
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1532 - " + _("Could not validate payment options string"));
	}
	unsigned char paymentOptionsMask = (unsigned char) GetPaymentOptionsMaskFromString(paymentOptions);
	...
	try {
		...
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1533 - " + _("Invalid price and/or quantity values. Quantity must be less than 4294967296 and greater than or equal to -1"));
	}
	...
	if (!GetTxOfOffer( vchOffer, theOffer, tx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1534 - " + _("Could not find an offer with this guid"));

	if (!GetTxOfAlias(theOffer.vchAlias, alias, aliastx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1535 - " + _("Could not find an alias with this name"));
	if (!vchAlias.empty() &&  !GetTxOfAlias(vchAlias, linkAlias, linkaliastx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1536 - " + _("Could not find an alias with this name"));

	if(!IsMyAlias(alias)) {
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1537 - " + _("This alias is not yours"));
	}
	...
	if(!vchCert.empty())
	{
		...
		if (!GetVtxOfCert( vchCert, theCert, vtxCert))
		{
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1538 - " + _("Updating an offer with a cert that does not exist"));
		}
		else if(theOffer.vchLinkOffer.empty() && theCert.vchAlias != theOffer.vchAlias)
		{
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1539 - " + _("Cannot update this offer because the certificate alias does not match the offer alias"));
		}
		if(!boost::algorithm::starts_with(stringFromVch(vchCat), "certificates"))
		{
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1540 - " + _("Offer selling a certificate must use a certificate category"));
		}
	}
	else if(boost::algorithm::starts_with(stringFromVch(vchCat), "certificates"))
	{
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1541 - " + _("Offer not selling a certificate cannot use a certificate category"));
	}
	if(!theOffer.vchLinkOffer.empty())
	{
		COffer linkOffer;
		...
		if (!GetVtxOfOffer( theOffer.vchLinkOffer, linkOffer, offerVtxPos))
		{
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1542 - " + _("Linked offer not found. It may be expired"));
		}
		else if(!linkOffer.linkWhitelist.GetLinkEntryByHash(vchAlias, entry))
		{
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1543 - " + _("Cannot find this alias in the parent offer affiliate list"));
		}
		if (!linkOffer.vchLinkOffer.empty())
		{
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1544 - " + _("Cannot link to an offer that is already linked to another offer"));
		}
		else if(linkOffer.sCategory.size() > 0 && boost::algorithm::starts_with(stringFromVch(linkOffer.sCategory), "wanted"))
		{
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1545 - " + _("Cannot link to a wanted offer"));
		}
	}
	...
	{
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1546 - " + _("Cannot change category to wanted"));
	}
	COutPoint outPoint;
	int numResults  = aliasunspent(theOffer.vchAlias, outPoint);	
	wtxAliasIn = pwalletMain->GetWalletTx(outPoint.hash);
	if (wtxAliasIn == NULL)
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1547 - " + _("This alias is not in your wallet"));
	...
	if(!vchAlias.empty() && vchAlias != theOffer.vchAlias)
	{
		...
		if (wtxLinkAliasIn == NULL)
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1548 - " + _("This alias is not in your wallet"));
	...
	if(offerCopy.vchLinkOffer.empty())
	{
		...
		int precision = 2;
		nPricePerUnit = convertCurrencyCodeToPresidentielcoin(alias.vchAliasPeg, sCurrencyCode, fPrice, chainActive.Tip()->nHeight, precision);
		if(nPricePerUnit == 0)
		{
			string err = "PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1549 - " + _("Could not find currency in the peg alias");
			throw runtime_error(err.c_str());
		}
	}
	if(params.size() >= 13 && !params[12].get_str().empty() && params[12].get_str() != "NONE")
	...
	if(!vchAlias.empty() && vchAlias != theOffer.vchAlias)
	{
		...
		CPresidentielcoinAddress linkAliasAddress;
	...
	SendMoneyPresidentielcoin(vecSend, recipient.nAmount+aliasRecipient.nAmount+fee.nAmount, false, wtx, wtxAliasIn, outPoint.n, alias.multiSigInfo.vchAliases.size() > 0 || linkAlias.multiSigInfo.vchAliases.size() > 0, NULL, wtxLinkAliasIn, outPointLink.n);
	UniValue res(UniValue::VARR);
...
UniValue offeraccept(const UniValue& params, bool fHelp) {
	if (fHelp || 1 > params.size() || params.size() > 6)
		throw runtime_error("offeraccept <alias> <guid> [quantity] [message] [Ext TxId] [payment option=PRC]\n"
				...
				"<paymentOption> If Ext TxId is defined, specify a valid payment option used to make payment. Default is PRC.\n"
				+ HelpRequiringPassphrase());
	CPresidentielcoinAddress refundAddr;
	vector<unsigned char> vchAlias = vchFromValue(params[0]);
	vector<unsigned char> vchOffer = vchFromValue(params[1]);
	...
	if (params.size() >= 3) {
		try {
			...
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1550 - " + _("Quantity must be less than 4294967296"));
		}
	}
	// payment options - get payment options string if specified otherwise default to PRC
	string paymentOptions = "PRC";
	...
	{
		// TODO change error number to something unique
		string err = "PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1551 - " + _("Could not validate the payment options value");
		throw runtime_error(err.c_str());
	}
		// payment options - and convert payment options string to a bitmask for the txn
	unsigned char paymentOptionsMask = (unsigned char) GetPaymentOptionsMaskFromString(paymentOptions);
	...
	vector<unsigned char> vchAccept = vchFromString(GeneratePresidentielcoinGuid());
	...
	{
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1552 - " + _("Could not find an offer with this identifier"));
	}
	...
	if(!theOffer.vchLinkOffer.empty() && !GetTxAndVtxOfOffer( theOffer.vchLinkOffer, linkOffer, linkedTx, vtxLinkPos))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1553 - " + _("Could not get linked offer"));
	...
	if(!GetTxAndVtxOfAlias(theOffer.vchAlias, theAlias, aliastx, aliasVtxPos, isExpired))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1554 - " + _("Could not find the alias associated with this offer"));

	CAliasIndex buyerAlias;
	if (!GetTxOfAlias(vchAlias, buyerAlias, buyeraliastx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1555 - " + _("Could not find buyer alias with this name"));
	...
	{
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1556 - " + _("Could not find currency in the peg alias"));
	}
	CCert theCert;
	...
	if(!theOffer.vchCert.empty())
	{
		...
		{
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1557 - " + _("Cannot purchase an expired certificate"));
		}
		else if(theOffer.vchLinkOffer.empty())
		{
			if(theCert.vchAlias != theOffer.vchAlias)
			{
				throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1558 - " + _("Cannot purchase this offer because the certificate has been transferred or it is linked to another offer"));
			}
		}
	}
	else if (vchMessage.size() <= 0)
	{
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1559 - " + _("Offer payment message cannot be empty"));
	}
	if(!theOffer.vchLinkOffer.empty())
	{
		if (!linkOffer.vchLinkOffer.empty())
		{
			 throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1560 - " + _("Cannot purchase offers that are linked more than once"));
		}
		vector<CAliasIndex> vtxAlias;
		CAliasIndex linkAlias;
		if(!GetVtxOfAlias(linkOffer.vchAlias, linkAlias, vtxAlias, isExpired))
		{
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1561 - " + _("Cannot find alias for this linked offer"));
		}
		else if(!theOffer.vchCert.empty() && theCert.vchAlias != linkOffer.vchAlias)
		{
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1562 - " + _("Cannot purchase this linked offer because the certificate has been transferred or it is linked to another offer"));
		}
		else if(linkOffer.sCategory.size() > 0 && boost::algorithm::starts_with(stringFromVch(linkOffer.sCategory), "wanted"))
		{
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1563 - " + _("Cannot purchase a wanted offer"));
		}
	}
	const CWalletTx *wtxAliasIn = NULL;
	COfferLinkWhitelistEntry foundEntry;
	...
	if(!theOffer.vchLinkOffer.empty())
	{
		if (!GetTxOfAlias(linkOffer.vchAlias, theLinkedAlias, aliastx))
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1564 - " + _("Could not find an alias with this name"));
		...
		if(!EncryptMessage(theLinkedAlias, vchMessage, strCipherText))
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1565 - " + _("Could not encrypt message to seller"));
	}
	else
	{
		// encrypt to offer owner
		if(!EncryptMessage(theAlias, vchMessage, strCipherText))
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1566 - " + _("Could not encrypt message to seller"));
	}
	...
	CPresidentielcoinAddress currentAddress;
	GetAddress(theAlias, &currentAddress, scriptPubKeyOrig);

	CPresidentielcoinAddress linkAddress;
	...
	SendMoneyPresidentielcoin(vecSend, acceptRecipient.nAmount+paymentRecipient.nAmount+fee.nAmount+aliasRecipient.nAmount, false, wtx, wtxAliasIn, outPoint.n, buyerAlias.multiSigInfo.vchAliases.size() > 0);
...
void HandleAcceptFeedback(const CFeedback& feedback, COffer& offer, vector<COffer> &vtxPos)
{
	if(feedback.nRating > 0)
	{
		...
		CPresidentielcoinAddress address = CPresidentielcoinAddress(aliasStr);
   ...
   // gather & validate inputs
	...
	try {
		...
		if(nRating < 0 || nRating > 5)
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1567 - " + _("Invalid rating value, must be less than or equal to 5 and greater than or equal to 0"));

	} catch (std::exception &e) {
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1568 - " + _("Invalid rating value"));
	}
	...
	if (!GetTxOfOffer( vchOffer, tmpOffer, tx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1569 - " + _("Could not find an offer with this guid"));

	
	if (!GetTxOfOfferAccept(tmpOffer.vchOffer, vchAcceptRand, theOffer, theOfferAccept, tx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1570 - " + _("Could not find this offer purchase"));

	vector<vector<unsigned char> > vvch;
	int op, nOut;
	if (!DecodeOfferTx(tx, op, nOut, vvch) || op != OP_OFFER_ACCEPT)
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1571 - " + _("Offer purchase transaction of wrong type"));

	if(vvch[0] != theOffer.vchOffer)
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1572 - " + _("Only merchant of this offer can leave feedback for this purchase"));
	...
	if(!GetTxOfAlias(theOfferAccept.vchBuyerAlias, buyerAlias, buyeraliastx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1573 - " + _("Could not buyer alias"));
	if(!GetTxOfAlias(theOffer.vchAlias, sellerAlias, selleraliastx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1574 - " + _("Could not merchant alias"));
	CPresidentielcoinAddress buyerAddress;
	...
	try
	{
		...
		if (!buyerAddress.GetKeyID(keyID))
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1575 - " + _("Buyer address does not refer to a key"));
		CKey vchSecret;
		if (!pwalletMain->GetKey(keyID, vchSecret))
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1576 - " + _("Private key for buyer address is not known"));
	...
	try
	{
		...
		if (!sellerAddress.GetKeyID(keyID))
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1577 - " + _("Seller address does not refer to a key"));
		CKey vchSecret;
		if (!pwalletMain->GetKey(keyID, vchSecret))
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1578 - " + _("Private key for seller address is not known"));
	...
	if(foundBuyerKey)
	{
		...
		wtxAliasIn = pwalletMain->GetWalletTx(outPoint.hash);
		if (wtxAliasIn == NULL)
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1579 - " + _("Buyer alias is not in your wallet"));
		scriptPubKeyAlias << CScript::EncodeOP_N(OP_ALIAS_UPDATE) << buyerAlias.vchAlias << buyerAlias.vchGUID << vchFromString("") << OP_2DROP << OP_2DROP;
	...
	else if(foundSellerKey)
	{
		...
		wtxAliasIn = pwalletMain->GetWalletTx(outPoint.hash);
		if (wtxAliasIn == NULL)
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1580 - " + _("Seller alias is not in your wallet"));
	...
	{
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1581 - " + _("You must be either the buyer or seller to leave feedback on this offer purchase"));
	}
	...
	SendMoneyPresidentielcoin(vecSend, recipient.nAmount+recipientAlias.nAmount+fee.nAmount, false, wtx, wtxAliasIn, outPoint.n, theAlias.multiSigInfo.vchAliases.size() > 0);
	UniValue res(UniValue::VARR);
	if(theAlias.multiSigInfo.vchAliases.size() > 0)
	{
		UniValue signParams(UniValue::VARR);
...
UniValue offeracceptacknowledge(const UniValue& params, bool fHelp) {
   ...
   // gather & validate inputs
	...
	if (!GetTxOfOffer( vchOffer, theOffer, tx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1582 - " + _("Could not find an offer with this guid"));
	...
	if(!theOffer.vchLinkOffer.empty())
	{
		if (!GetTxOfOffer( theOffer.vchLinkOffer, theOffer, linkTx))
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1583 - " + _("Could not find a linked offer with this guid"));
		if(!GetTxOfAlias(theOffer.vchAlias, sellerAlias, selleraliastx))
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1584 - " + _("Could not find merchant alias"));
	}
	else
	{
		if(!GetTxOfAlias(theOffer.vchAlias, sellerAlias, selleraliastx))
			throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1585 - " + _("Could not find merchant alias"));
	}
	...
	if (!GetTxOfOfferAccept(theOffer.vchOffer, vchAcceptRand, tmpOffer, theOfferAccept, tx, true))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1586 - " + _("Could not find this offer purchase"));
	...
	if(!GetTxOfAlias(theOfferAccept.vchBuyerAlias, buyerAlias, buyeraliastx))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1587 - " + _("Could not buyer alias"));
	CPresidentielcoinAddress buyerAddress;
	GetAddress(buyerAlias, &buyerAddress, buyerScript);
	...
	CKeyID keyID;
	if (!sellerAddress.GetKeyID(keyID))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1588 - " + _("Seller address does not refer to a key"));
	CKey vchSecret;
	if (!pwalletMain->GetKey(keyID, vchSecret))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1589 - " + _("Private key for seller address is not known"));
	...
	wtxAliasIn = pwalletMain->GetWalletTx(outPoint.hash);
	if (wtxAliasIn == NULL)
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1590 - " + _("Seller alias is not in your wallet"));
	...
	SendMoneyPresidentielcoin(vecSend, recipientBuyer.nAmount+recipientAlias.nAmount+fee.nAmount, false, wtx, wtxAliasIn, outPoint.n, sellerAlias.multiSigInfo.vchAliases.size() > 0);
	UniValue res(UniValue::VARR);
...
UniValue offerinfo(const UniValue& params, bool fHelp) {
	...
	vector<COffer> vtxPos;
	if (!pofferdb->ReadOffer(vchOffer, vtxPos) || vtxPos.empty())
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR: ERRCODE: 1591 - " + _("Failed to read from offer DB"));

	// check that the seller isn't banned level 2
	CTransaction aliastx;
	CAliasIndex alias;
	if(!GetTxOfAlias(vtxPos.back().vchAlias, alias, aliastx, true))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1592 - " + _("Could not find the alias associated with this offer"));

	if(!BuildOfferJson(vtxPos.back(), alias, oOffer))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1593 - " + _("Could not find this offer"));
...
bool BuildOfferJson(const COffer& theOffer, const CAliasIndex &alias, UniValue& oOffer, const string &strPrivKey)
{
	...
	int precision = 2;
	CAmount nPricePerUnit = convertPresidentielcoinToCurrencyCode(alias.vchAliasPeg, theOffer.sCurrencyCode, theOffer.GetPrice(), nHeight, precision);
...
UniValue offeracceptlist(const UniValue& params, bool fHelp) {
	...
	if(aliases.size() > 0)
	{
		for(unsigned int aliasIndex =0;aliasIndex<aliases.size();aliasIndex++)
		{
			...
			CTransaction aliastx;
			uint256 txHash;
			if (!GetPresidentielcoinTransaction(alias.nHeight, alias.txHash, aliastx, Params().GetConsensus()))
				continue;
			...
			for(std::vector<CAliasIndex>::reverse_iterator it = vtxPos.rbegin(); it != vtxPos.rend(); ++it) {
				...
				if(!GetPresidentielcoinTransaction(theAlias.nHeight, theAlias.txHash, tx, Params().GetConsensus()))
					continue;
				vector<vector<unsigned char> > vvch;
				int op, nOut;
...
bool BuildOfferAcceptJson(const COffer& theOffer, const CAliasIndex& theAlias, const CTransaction &aliastx, UniValue& oOfferAccept, const string &strPrivKey)
{
	...
	vector<vector<unsigned char> > vvch;
	int op, nOut;
	if(!GetPresidentielcoinTransaction(theOffer.nHeight, theOffer.txHash, offerTx, Params().GetConsensus()))
		return false;
	...
	CAmount nPricePerUnit = convertPresidentielcoinToCurrencyCode(theAlias.vchAliasPeg, theOffer.sCurrencyCode, priceAtTimeOfAccept, theOffer.accept.nAcceptHeight, precision);
	CAmount nPricePerUnitExt = 0;
	if(theOffer.accept.nPaymentOption != PAYMENTOPTION_SYS)
		nPricePerUnitExt = convertPresidentielcoinToCurrencyCode(theAlias.vchAliasPeg, vchFromString(GetPaymentOptionsString(theOffer.accept.nPaymentOption)), priceAtTimeOfAccept, theOffer.accept.nAcceptHeight, extprecision);
	...
	oOfferAccept.push_back(Pair("ismine", IsPresidentielcoinTxMine(aliastx, "alias")? "true" : "false"));
...
UniValue offerhistory(const UniValue& params, bool fHelp) {
	...
	vector<COffer> vtxPos;
	if (!pofferdb->ReadOffer(vchOffer, vtxPos) || vtxPos.empty())
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1594 - " + _("Failed to read from offer DB"));

	vector<CAliasIndex> vtxAliasPos;
	if (!paliasdb->ReadAlias(vtxPos.back().vchAlias, vtxAliasPos) || vtxAliasPos.empty())
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1595 - " + _("Failed to read from alias DB"));
	...
	BOOST_FOREACH(txPos2, vtxPos) {
		...
		if(!paliasdb->ReadAlias(txPos2.vchAlias, vtxAliasPos) || vtxAliasPos.empty())
			continue;
		if (!GetPresidentielcoinTransaction(txPos2.nHeight, txPos2.txHash, tx, Params().GetConsensus())) {
			continue;
		}
		if (!DecodeOfferTx(tx, op, nOut, vvch) )
			continue;
...
UniValue offerfilter(const UniValue& params, bool fHelp) {
	...
	if (!pofferdb->ScanOffers(vchOffer, strRegexp, safeSearch, strCategory, 25, offerScan))
		throw runtime_error("PRESIDENTIELCOIN_OFFER_RPC_ERROR ERRCODE: 1596 - " + _("Scan failed"));
	CTransaction aliastx;
...
std::string GetPaymentOptionsString(const uint32_t paymentOptions)
{
	...
	if(IsPaymentOptionInMask(paymentOptions, PAYMENTOPTION_SYS)) {
		currencies.push_back(std::string("PRC"));
	}
