...
using namespace std;
// PRESIDENTIELCOIN services
extern int IndexOfAliasOutput(const CTransaction& tx);
extern bool IsPresidentielcoinScript(const CScript& scriptPubKey, int &op, vector<vector<unsigned char> > &vvchArgs);
extern int GetPresidentielcoinTxVersion();
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CPresidentielcoinAddress(CScriptID(redeemScript)).ToString();
                                 ...
                                 set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet) const
{
    ...
    BOOST_FOREACH(const COutput &output, vCoins)
    {
		...
		// PRESIDENTIELCOIN txs are unspendable unless input to another presidentielcoin tx (passed into createtransaction)
		if(pcoin->nVersion == GetPresidentielcoinTxVersion())
		{
			int op;
			vector<vector<unsigned char> > vvchArgs;
			if (pcoin->vout.size() >= i && IsPresidentielcoinScript(pcoin->vout[i].scriptPubKey, op, vvchArgs) && op != OP_ALIAS_PAYMENT)
				continue;
		}
    ...
    BOOST_FOREACH(const COutPoint& outpoint, vPresetInputs)
    {
        ...
        if (it != mapWallet.end())
        {
			...
			if(pcoin->nVersion == GetPresidentielcoinTxVersion())
			{
				...
				if (pcoin->vout.size() >= outpoint.n && IsPresidentielcoinScript(pcoin->vout[outpoint.n].scriptPubKey, op, vvchArgs) && op != OP_ALIAS_PAYMENT)
					continue;
			}
...
{
    CAmount nValue = 0;
	// PRESIDENTIELCOIN: get output amount of input transactions for presidentielcoin service calls
	if(wtxInAlias != NULL)
	{
		if (nTxOutAlias < 0)
		{
			strFailReason = _("Can't determine type of alias input into presidentielcoin service transaction");
            return false;
		}
	}
	if(wtxInLinkAlias != NULL)
	{
		...
		{
			strFailReason = _("Can't determine type of linked alias input into presidentielcoin service transaction");
            return false;
		}
	}
	...
	// PRESIDENTIELCOIN: set presidentielcoin tx version if its a presidentielcoin service call
	if(sysTx)
		txNew.nVersion = GetPresidentielcoinTxVersion();
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                BOOST_FOREACH (const CRecipient& recipient, vecSend)
                {
					// PRESIDENTIELCOIN pay to alias
					CRecipient myrecipient = recipient;
					CTxDestination payDest;
					int op;
					vector<vector<unsigned char> > vvchArgs;
					if (!myrecipient.scriptPubKey.IsUnspendable() && !IsPresidentielcoinScript(myrecipient.scriptPubKey, op, vvchArgs)) {
						if (ExtractDestination(myrecipient.scriptPubKey, payDest)) 
						{
							CPresidentielcoinAddress address = CPresidentielcoinAddress(payDest);
							address = CPresidentielcoinAddress(address.ToString());
							if(address.isAlias)
							{
				...
				// PRESIDENTIELCOIN input credit from input tx
                ...
                CAmount nValueIn = 0;
				// PRESIDENTIELCOIN add input credit to current coin selection
				...
				// PRESIDENTIELCOIN attach input TX
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-presidentielcoin-address
                    ...
                    if (coinControl && !boost::get<CNoDestination>(&coinControl->destChange))
					{
						...
						address = CPresidentielcoinAddress(coinControl->destChange);
						address = CPresidentielcoinAddress(address.ToString());
						if(!address.vchRedeemScript.empty())
                    ...
                    else
                    {
                       // Note: We use a new key here to keep it from being obvious which side is the change.
						...
						// PRESIDENTIELCOIN pay to input destination as change
						CTxDestination payDest;
						...
						if (ExtractDestination(vecCoins[nLastIndex].first->vout[vecCoins[nLastIndex].second].scriptPubKey, payDest)) 
						{
							...
							address = CPresidentielcoinAddress(payDest);
							address = CPresidentielcoinAddress(address.ToString());
					...
					// PRESIDENTIELCOIN change as a alias payment				
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CPresidentielcoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CPresidentielcoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CPresidentielcoinAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CPresidentielcoinAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CPresidentielcoinAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CPresidentielcoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CPresidentielcoinAddress(dest).ToString(), key);
}
