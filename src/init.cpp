 ...
 * between presidentielcoind, and presidentielcoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("presidentielcoin-shutoff");
...

    {
		...
		if(!fTxIndex)
		{
			...
			LogPrintf("%s: Cleaning up Presidentielcoin Databases...\n", __func__);
			CleanupPresidentielcoinServiceDatabases(servicesCleaned);
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), PRESIDENTIELCOIN_CONF_FILENAME));
    if (mode == HMM_PRESIDENTIELCOIND)
    {
#ifndef WIN32
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), PRESIDENTIELCOIN_PID_FILENAME));
#endif
    ...
    if (mode == HMM_PRESIDENTIELCOIN_QT)
        debugCategories += ", qt";
...
{
	// PRESIDENTIELCOIN eula
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Presidentielcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           ...
           "\n" +
           FormatParagraph(_("Presidentielcoin is open source software produced by a global network of developers. By downloading, distributing and using Presidentielcoin and the Presidentielcoin network you release the developers involved in the Presidentielcoin Project past, present, and future from any and all liability. You are responsible for your creations on the Presidentielcoin network. You agree that the developers of the Presidentielcoin Project carry no responsibility for the actions/data or entities of *any* definition created on the network by yourself or others on the network to which you may be exposed.")) + "\n" +
           "\n" +
           FormatParagraph(_("The developers of the Presidentielcoin Project do not have the power to modify data on the Presidentielcoin network, it is backed by an immutable blockchain, which you further acknowledge through use of Presidentielcoin, the Presidentielcoin network, and Presidentielcoin services. If you do not agree to these terms, please refrain from using Presidentielcoin and its related services.")) +
           "\n";
}
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("presidentielcoin-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that Presidentielcoin is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Presidentielcoin version %s\n", FormatFullVersion());
}

/** Initialize presidentielcoin.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Make sure only a single Presidentielcoin process is using the data directory.
