// Copyright (c) 2014-2015 The Presidentielcoin Core developers
...
// PRESIDENTIELCOIN use aliases as addresses
...
namespace
{
class CPresidentielcoinAddressVisitor : public boost::static_visitor<bool>
{
private:
    CPresidentielcoinAddress* addr;
	// PRESIDENTIELCOIN support old sys
	CChainParams::AddressType nSysVer;
public:
    CPresidentielcoinAddressVisitor(CPresidentielcoinAddress* addrIn) : addr(addrIn) {}
	CPresidentielcoinAddressVisitor(CPresidentielcoinAddress* addrIn, CChainParams::AddressType nSysVer) : nSysVer(nSysVer), addr(addrIn) {}
...
// PRESIDENTIELCOIN aliases as addresses
CPresidentielcoinAddress::CPresidentielcoinAddress() {
	isAlias = false;
	aliasName = "";
	safeSearch = false;
	safetyLevel = 0;
...
// PRESIDENTIELCOIN support old sys
CPresidentielcoinAddress::CPresidentielcoinAddress(const CTxDestination &dest, CChainParams::AddressType sysVer) { 
	isAlias = false;
	safeSearch = false;
	safetyLevel = 0;
	aliasName = "";
...
CPresidentielcoinAddress::CPresidentielcoinAddress(const std::string& strAddress) { 
	isAlias = false;
	aliasName = "";
...
CPresidentielcoinAddress::CPresidentielcoinAddress(const char* pszAddress) { 
	isAlias = false;
...
// PRESIDENTIELCOIN support old sys
bool CPresidentielcoinAddress::Set(const CKeyID& id, CChainParams::AddressType sysVer)
{   
...
bool CPresidentielcoinAddress::Set(const CScriptID& id, CChainParams::AddressType sysVer)
{
...
// PRESIDENTIELCOIN support old sys
bool CPresidentielcoinAddress::Set(const CTxDestination& dest, CChainParams::AddressType sysVer)
{
    return boost::apply_visitor(CPresidentielcoinAddressVisitor(this, sysVer), dest);
}

bool CPresidentielcoinAddress::IsValid() const
{
    return IsValid(Params());
}

bool CPresidentielcoinAddress::IsValid(const CChainParams& params) const
{
    bool fCorrectSize = vchData.size() == 20;
	// PRESIDENTIELCOIN allow old PRESIDENTIELCOIN address scheme
...
CTxDestination CPresidentielcoinAddress::Get() const
{
    if (!IsValid())
...
bool CPresidentielcoinAddress::GetKeyID(CKeyID& keyID) const
{
	// PRESIDENTIELCOIN allow old PRESIDENTIELCOIN address scheme
...
bool CPresidentielcoinAddress::IsScript() const
{
...
void CPresidentielcoinSecret::SetKey(const CKey& vchSecret)
{
...
CKey CPresidentielcoinSecret::GetKey()
{
    CKey ret;
...
bool CPresidentielcoinSecret::IsValid() const
{
...
bool CPresidentielcoinSecret::SetString(const char* pszSecret)
{
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CPresidentielcoinSecret::SetString(const std::string& strSecret)
{
