    ...
    ui(new Ui::EditOfferDialog), mapper(0), mode(mode), model(0)
{
	...
	ui->currencyDisclaimer->setText(QString("<font color='blue'>") + tr("You will receive payment in Presidentielcoin equivalent to the Market-value of the currency you have selected") + QString("</font>"));
	ui->paymentOptionsDisclaimer->setText(QString("<font color='blue'>") + tr("Choose which crypto-currency you want to allow as a payment method for this offer. Your choices are any combination of PRC, BTC or ZEC. An example setting for all three: 'PRC+BTC+ZEC'. For PRC and ZEC: 'PRC+ZEC'. Please note that in order to spend coins paid to you via Presidentielcoin Marketplace, you will need to import your Presidentielcoin private key in external wallet(s) if BTC or ZEC are chosen.") + QString("</font>"));
	cert = strCert;
	alias = strAlias;
	ui->certEdit->clear();
    ...
    if(!saveCurrentRow())
    {
		if(model)
		{
			...
			case OfferTableModel::INVALID_OFFER:
				QMessageBox::warning(this, windowTitle(),
					tr("The entered offer is not a valid Presidentielcoin offer"),
					QMessageBox::Ok, QMessageBox::Ok);
				break;
