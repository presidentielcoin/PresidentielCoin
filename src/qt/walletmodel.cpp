...
{
    CPresidentielcoinAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered presidentielcoin address / amount:
            ...
            CScript scriptPubKey = GetScriptForDestination(CPresidentielcoinAddress(rcp.address.toStdString()).Get());
...

    {
		...
		// PRESIDENTIELCOIN pass in false to not sign
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        Q_FOREACH(const SendCoinsRecipient &rcp, transaction.getRecipients())
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal presidentielcoin:URI (presidentielcoin:123...?message=example)
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, transaction.getRecipients())
    {
        ...
        if (!rcp.paymentRequest.IsInitialized())
        {
            ...
            CTxDestination dest = CPresidentielcoinAddress(strAddress).Get();
...
// PRESIDENTIELCOIN
static void NotifyPresidentielcoinTransactionChanged(WalletModel *walletmodel, const CTransaction &tx, ChangeType status)
{
...
{
    QString strAddress = QString::fromStdString(CPresidentielcoinAddress(address).ToString());
...
static void NotifyTransactionChanged(WalletModel *walletmodel, CWallet *wallet, const uint256 &hash, ChangeType status)
{
	...
	if(inWallet)
		NotifyPresidentielcoinTransactionChanged(walletmodel, mi->second, status);
}
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
		...
		if(out.tx->nVersion == GetPresidentielcoinTxVersion())
		{
			int op;
			vector<vector<unsigned char> > vvchArgs;
			// any prc tx thats not an alias payment shouldnt show up in listCoins (used by coin control)
			if (out.tx->vout.size() >= out.i && IsPresidentielcoinScript(out.tx->vout[out.i].scriptPubKey, op, vvchArgs) && op != OP_ALIAS_PAYMENT)
				continue;
		}
        ...
        if(/*!out.fSpendable || */!ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
		// PRESIDENTIELCOIN
		CPresidentielcoinAddress presidentielcoinAddress = CPresidentielcoinAddress(address);
		presidentielcoinAddress = CPresidentielcoinAddress(presidentielcoinAddress.ToString());
		QString qStrAddress = QString::fromStdString(presidentielcoinAddress.ToString());
		if(presidentielcoinAddress.isAlias)
			qStrAddress = QString::fromStdString(presidentielcoinAddress.aliasName);
...
{
    CTxDestination dest = CPresidentielcoinAddress(sAddress).Get();
