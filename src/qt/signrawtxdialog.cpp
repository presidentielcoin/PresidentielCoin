...
{
	ui->setupUi(this);
	ui->rawTxDisclaimer->setText(QString("<font color='blue'>") + tr("Sign a raw presidentielcoin transaction and send it to the network if it is complete with all required signatures. Enter the raw hex encoded transaction below") + QString("</font>"));
	...
	ui->decodeSysTxDisclaimer->setText(QString("<font color='blue'>") + tr("The area below is to display presidentielcoin specific information regarding this transaction. Currently there is nothing to display") + QString("</font>"));
...
void SignRawTxDialog::setRawSysTxEdit()
{
	...
	UniValue arraySendParams(UniValue::VARR);
	string strMethod;
	strMethod = string("presidentielcoindecoderawtransaction");
	params.push_back(ui->rawTxEdit->toPlainText().toStdString());
	...
	try {
		...
		ui->decodeSysTxDisclaimer->setText(QString("<font color='blue'>") + tr("The area below is to display presidentielcoin specific information regarding this transaction") + QString("</font>"));
	}
	catch (UniValue& objError)
	{
		string strError = find_value(objError, "message").get_str();
		ui->rawSysTxDecodeEdit->setText(tr("Error creating decoding raw presidentielcoin transaction: ") + QString::fromStdString(strError));
	}
	catch(std::exception& e)
	{
		ui->rawSysTxDecodeEdit->setText(tr("General exception decoding raw presidentielcoin transaction"));
	}	
}
...
bool SignRawTxDialog::saveCurrentRow()
{
	...
	UniValue arraySendParams(UniValue::VARR);
	string strMethod;
	strMethod = string("presidentielcoinsignrawtransaction");
	params.push_back(ui->rawTxEdit->toPlainText().toStdString());
