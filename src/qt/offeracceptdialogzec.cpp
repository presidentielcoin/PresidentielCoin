...
#include "config/presidentielcoin-config.h" /* for USE_QRCODE */
#endif
#ifdef USE_QRCODE
#include <qrencode.h>
#endif
    ...
    ui(new Ui::OfferAcceptDialogZEC), platformStyle(platformStyle), alias(alias), offer(offer), notes(notes), quantity(quantity), title(title), sellerAlias(sellerAlias), address(address), arbiter(arbiter)
{
    ...
    int zecprecision;
    CAmount zecPrice = convertPresidentielcoinToCurrencyCode(vchFromString(strAliasPeg.toStdString()), vchFromString("ZEC"), AmountFromValue(sysPrice.toStdString()), chainActive.Tip()->nHeight, zecprecision);
	if(zecPrice > 0)
	...
	ui->escrowDisclaimer->setText(QString("<font color='blue'>") + tr("Enter a Presidentielcoin arbiter that is mutally trusted between yourself and the merchant. Then enable the 'Use Escrow' checkbox") + QString("</font>"));
...
#ifdef USE_QRCODE
	QString message = tr("Payment on Presidentielcoin Decentralized Marketplace. Offer ID: ") + this->offer;
