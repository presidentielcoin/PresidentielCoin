...
const std::string PresidentielcoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString PresidentielcoinGUI::DEFAULT_WALLET = "~Default";

PresidentielcoinGUI::PresidentielcoinGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
PresidentielcoinGUI::~PresidentielcoinGUI()
{
...
void PresidentielcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Presidentielcoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and presidentielcoin: URIs)"));
	...
	// PRESIDENTIELCOIN
    aboutAction = new QAction(QIcon(":/icons/" + theme + "/about"), tr("&About Presidentielcoin Core"), this);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Presidentielcoin addresses to prove you own them"));
	// PRESIDENTIELCOIN
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/" + theme + "/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Presidentielcoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a presidentielcoin: URI or payment request"));

	// PRESIDENTIELCOIN
    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/" + theme + "/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Presidentielcoin command-line options").arg(tr(PACKAGE_NAME)));
...
void PresidentielcoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void PresidentielcoinGUI::createToolBars()
{
    if(walletFrame)
    {
...
void PresidentielcoinGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool PresidentielcoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool PresidentielcoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void PresidentielcoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void PresidentielcoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void PresidentielcoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void PresidentielcoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void PresidentielcoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void PresidentielcoinGUI::optionsClicked()
{
...
void PresidentielcoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void PresidentielcoinGUI::showDebugWindow()
{
...
void PresidentielcoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void PresidentielcoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void PresidentielcoinGUI::openClicked()
{
...
void PresidentielcoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void PresidentielcoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void PresidentielcoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void PresidentielcoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void PresidentielcoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void PresidentielcoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
// PRESIDENTIELCOIN
void PresidentielcoinGUI::gotoAliasListPage()
{
    if (walletFrame) walletFrame->gotoAliasListPage();
}

void PresidentielcoinGUI::gotoMessageListPage()
{
    if (walletFrame) walletFrame->gotoMessageListPage();
}
void PresidentielcoinGUI::gotoEscrowListPage()
{
    if (walletFrame) walletFrame->gotoEscrowListPage();
}
void PresidentielcoinGUI::gotoOfferListPage()
{
    if (walletFrame) walletFrame->gotoOfferListPage();
}

void PresidentielcoinGUI::gotoCertListPage()
{
    if (walletFrame) walletFrame->gotoCertListPage();
}
#endif // ENABLE_WALLET

void PresidentielcoinGUI::setNumConnections(int count)
{
    QString icon;
	// PRESIDENTIELCOIN
    QString theme = GUIUtil::getThemeName();
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Presidentielcoin network", "", count));
}

void PresidentielcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
...
void PresidentielcoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Presidentielcoin"); // default title
    ...
    // Append title to "Presidentielcoin - "
    if (!msgType.isEmpty())
        strTitle += " - " + msgType;
...
void PresidentielcoinGUI::changeEvent(QEvent *e)
{
...
void PresidentielcoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void PresidentielcoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void PresidentielcoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(PresidentielcoinUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void PresidentielcoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void PresidentielcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool PresidentielcoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool PresidentielcoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void PresidentielcoinGUI::setEncryptionStatus(int status)
{
	// PRESIDENTIELCOIN
	QString theme = GUIUtil::getThemeName();
    switch(status)
...
void PresidentielcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void PresidentielcoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void PresidentielcoinGUI::detectShutdown()
{
...
void PresidentielcoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void PresidentielcoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMessageBox(PresidentielcoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void PresidentielcoinGUI::subscribeToCoreSignals()
{
...
void PresidentielcoinGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<PresidentielcoinUnits::Unit> units = PresidentielcoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const PresidentielcoinUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(PresidentielcoinUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(PresidentielcoinUnits::Unit u, PresidentielcoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(PresidentielcoinUnits::name(u)), this);
...
{
    setText(PresidentielcoinUnits::name(newUnits));
}
