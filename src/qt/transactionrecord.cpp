...
extern bool IsPresidentielcoinDataOutput(const CTxOut& out);
extern bool IsPresidentielcoinTxMine(const CTransaction& tx, const string &type);
extern std::string stringFromVch(const std::vector<unsigned char> &vch);
extern bool DecodeAndParsePresidentielcoinTx(const CTransaction& tx, int& op, int& nOut, vector<vector<unsigned char> >& vvch);
enum {RECV=0, SEND=1};
static bool CreatePresidentielcoinTransactionRecord(TransactionRecord& sub, int op, const vector<vector<unsigned char> > &vvchArgs, const CWalletTx &wtx, int type)
{
	COffer offer;
	CEscrow escrow;
	switch(op)
	...
	case OP_ALIAS_UPDATE:
		if(type == SEND)
			sub.type = (IsPresidentielcoinTxMine(wtx, "alias")) ? TransactionRecord::AliasUpdate : TransactionRecord::AliasTransfer;	
		else if(type == RECV)
...
static bool CreatePresidentielcoinTransactions(const CWallet *wallet, const CWalletTx& wtx, QList<TransactionRecord>& parts, const int64_t &nTime, const CAmount &nNet, const int type)
{
	if(wtx.nVersion != GetPresidentielcoinTxVersion())
		return false;	
	uint256 hash = wtx.GetHash();
    vector<vector<unsigned char> > vvchArgs;
    int op, nOut;
	// there should only be one data carrying presidentielcoin output per transaction, but there may be more than 1 presidentielcoin utxo in a transaction
	...
	if(!DecodeAndParsePresidentielcoinTx(wtx, op, nOut, vvchArgs))
		return false;

	TransactionRecord sub(hash, nTime);
	if(!CreatePresidentielcoinTransactionRecord(sub, op, vvchArgs, wtx, type))
		return false;
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    if (nNet > 0 || wtx.IsCoinBase())
    {
        ...
        // Credit
        //
		// PRESIDENTIELCOIN - this should be a received service
		if(!CreatePresidentielcoinTransactions(wallet, wtx, parts, nTime, nNet, RECV))
		{
			BOOST_FOREACH(const CTxOut& txout, wtx.vout)
			{
				isminetype mine = wallet->IsMine(txout);
				if(mine)
				{
					...
					{
							// Received by Presidentielcoin Address
							sub.type = TransactionRecord::RecvWithAddress;
							// PRESIDENTIELCOIN show alias in record
							CPresidentielcoinAddress sysAddress = CPresidentielcoinAddress(address);
							sysAddress = CPresidentielcoinAddress(sysAddress.ToString());
    ...
    else
    {
        ...
        else if (fAllFromMe)
        {
            ...
            // Debit
            //
			// PRESIDENTIELCOIN - this should be a new service you've created
			if(!CreatePresidentielcoinTransactions(wallet, wtx, parts, nTime, nNet, SEND))
			{
				...
				for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
				{
					...
					{
						// Sent to Presidentielcoin Address
