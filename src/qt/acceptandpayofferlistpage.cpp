...
void AcceptandPayOfferListPage::OpenBTCPayDialog()
{
	...
	CAmount sysPrice = convertCurrencyCodeToPresidentielcoin(vchFromString(ui->aliasPegEdit->text().toStdString()), vchFromString(ui->infoCurrency->text().toStdString()), dblPrice, chainActive.Tip()->nHeight, sysprecision);
	if(sysPrice == 0)
	{
        QMessageBox::critical(this, windowTitle(),
