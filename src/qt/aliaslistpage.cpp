	...
	currentPage(0)
{
    ...
    ui->labelExplanation->setText(tr("Search for Presidentielcoin Aliases. Select Safe Search from wallet options if you wish to omit potentially offensive Aliases(On by default)"));
...
void AliasListPage::showEvent ( QShowEvent * event )
{
	...
	{
        ui->labelExplanation->setText(tr("<font color='blue'>WARNING: Your wallet is currently locked. For security purposes you'll need to enter your passphrase in order to search Presidentielcoin Aliases.</font> <a href=\"http://lockedwallet.presidentielcoin.org\">more info</a>"));
