    ...
    platformStyle(platformStyle)
{
    ...
    // see https://github.com/presidentielcoin/presidentielcoin2/issues/5716
...
{
    GUIUtil::setClipboard(PresidentielcoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = PresidentielcoinUnits::PRC;
    ...
    l2->setText(PresidentielcoinUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(PresidentielcoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(PresidentielcoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l6->setText(sPriorityLabel);                                             // Priority
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(PresidentielcoinUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0 && (coinControl->nMinimumTotalFee < nPayFee))
    {
    ...
    toolTip1 += tr("This means a fee of at least %1 per kB is required.").arg(PresidentielcoinUnits::formatHtmlWithUnit(nDisplayUnit, CWallet::GetRequiredFee(1000))) + "<br /><br />";
    toolTip1 += tr("Can vary +/- 1 byte per input.");

    QString toolTip2 = tr("Transactions with higher priority are more likely to get included into a block.") + "<br /><br />";
    toolTip2 += tr("This label turns red if the priority is smaller than \"medium\".") + "<br /><br />";
    toolTip2 += tr("This means a fee of at least %1 per kB is required.").arg(PresidentielcoinUnits::formatHtmlWithUnit(nDisplayUnit, CWallet::GetRequiredFee(1000)));
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            {
				// PRESIDENTIELCOIN
				CPresidentielcoinAddress presidentielcoinAddress = CPresidentielcoinAddress(outputAddress).ToString();
				sAddress = QString::fromStdString(presidentielcoinAddress.ToString());
				if(presidentielcoinAddress.isAlias)
					sAddress = QString::fromStdString(presidentielcoinAddress.aliasName);

                // if listMode or change => show presidentielcoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, PresidentielcoinUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, PresidentielcoinUnits::format(nDisplayUnit, nSum));
