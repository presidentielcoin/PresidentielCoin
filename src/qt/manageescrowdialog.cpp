    ...
    ui(new Ui::ManageEscrowDialog), escrow(escrow)
{
	...
	else if(status.indexOf("escrow released") == 0)
	{
		if(escrowRoleType == Buyer)
		{
			ui->manageInfo2->setText(tr("You are the 'buyer' of the offer held in escrow. The escrow has been released to the merchant. You may communicate with your arbiter or merchant via Presidentielcoin messages. You may leave feedback after the money is claimed by the merchant."));
			ui->refundButton->setEnabled(false);
			ui->releaseButton->setEnabled(false);
		}
		else if(escrowRoleType == Seller)
	...
	{
		ui->manageInfo2->setText(tr("The escrow status was not recognized. Please contact the Presidentielcoin team."));
		ui->refundButton->setEnabled(false);
		ui->releaseButton->setEnabled(false);
	}
}
