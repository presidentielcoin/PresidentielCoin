...
#if defined(NDEBUG)
# error "Presidentielcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Presidentielcoin Signed Message:\n";
...
// PRESIDENTIELCOIN
bool CheckPresidentielcoinInputs(const CTransaction& tx, const CCoinsViewCache& inputs, bool fJustCheck, int nHeight=0)
{
	...
	string errorMessage;
	if(tx.nVersion == GetPresidentielcoinTxVersion())
	{
		bool good = true;
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
	...
	// PRESIDENTIELCOIN
    if (fRequireStandard && tx.nVersion != GetPresidentielcoinTxVersion() && tx.nVersion >= 2 && VersionBitsTipState(chainparams.GetConsensus(), Consensus::DEPLOYMENT_CSV) != THRESHOLD_ACTIVE) {
...

    {
		...
		// PRESIDENTIELCOIN
        if (!CheckPresidentielcoinInputs(tx, view, true))
			return false;
...
//
// PRESIDENTIELCOIN check header auxpow proof of work
...
// PRESIDENTIELCOIN: block header template's for auxpow integration
template<typename T>
static bool ReadBlockOrHeader(T& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // PRESIDENTIELCOIN Check the header
...
// PRESIDENTIELCOIN disconnect service related blocks
...
void ThreadScriptCheck() {
    RenameThread("presidentielcoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    std::vector<int> prevheights;
	// PRESIDENTIELCOIN inflate when high demand for services
    ...
    {
        const CTransaction &tx = block.vtx[i];
		// PRESIDENTIELCOIN inflate and regenerate based on opreturn value set when creating the service(user)
		...
		if(tx.nVersion == GetPresidentielcoinTxVersion())
		{
			...
			if(nSysBlockTx >= 5)
			{
				int nOut = GetPresidentielcoinDataOutput(tx);
				if (nOut != -1)
        ...
        if (!tx.IsCoinBase())
        {
			...
			// PRESIDENTIELCOIN
			if (!CheckPresidentielcoinInputs(tx, view, fJustCheck, pindex->nHeight))
				return error("ConnectBlock(): CheckPresidentielcoinInputs on %s failed",tx.GetHash().ToString());
            control.Add(vChecks);
        }
	...
	// PRESIDENTIELCOIN Add dynamic inflation based on service demand
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        {
			// PRESIDENTIELCOIN getbaseversion
...
{
	// PRESIDENTIELCOIN check auxpow first then fallback to normal check
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex * const pindexPrev, int64_t nAdjustedTime)
{
	...
	// PRESIDENTIELCOIN use GetBaseVersion() because of CBlockVersion
                     ...
                     ++mi)
                {
                    ...
                    else if (!fMissingInputs2)
                    {
                        ...
                        if (orphanTx.wit.IsNull() && !stateDummy.CorruptionPossible()) {
                            ...
                            // See https://github.com/presidentielcoin/presidentielcoin2/issues/8279 for details.
                            assert(recentRejects);
