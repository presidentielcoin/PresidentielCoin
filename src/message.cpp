...
bool CheckMessageInputs(const CTransaction &tx, int op, int nOut, const vector<vector<unsigned char> > &vvchArgs, const CCoinsViewCache &inputs, bool fJustCheck, int nHeight, string &errorMessage, bool dontaddtodb) {
	...
	if (tx.nVersion != PRESIDENTIELCOIN_TX_VERSION)
	{
		errorMessage = "PRESIDENTIELCOIN_MESSAGE_CONSENSUS_ERROR: ERRCODE: 3000 - " + _("Non-Presidentielcoin transaction found");
		return true;
	}
	...
	vector<unsigned char> vchHash;
	int nDataOut;
	if(!GetPresidentielcoinData(tx, vchData, vchHash, nDataOut) || !theMessage.UnserializeFromData(vchData, vchHash))
	{
		errorMessage = "PRESIDENTIELCOIN_MESSAGE_CONSENSUS_ERROR ERRCODE: 3001 - " + _("Cannot unserialize data inside of this transaction relating to a message");
		return true;
	}
	...
	{	
		if(vvchArgs.size() != 2)
		{
			errorMessage = "PRESIDENTIELCOIN_MESSAGE_CONSENSUS_ERROR: ERRCODE: 3002 - " + _("Message arguments incorrect size");
		...
		if(!theMessage.IsNull())
		{
			...
			{
				errorMessage = "PRESIDENTIELCOIN_MESSAGE_CONSENSUS_ERROR: ERRCODE: 3003 - " + _("Hash provided doesn't match the calculated hash of the data");
				return true;
			}
		}
	...
	if(fJustCheck)
	{
		...
		{
			errorMessage = "PRESIDENTIELCOIN_MESSAGE_CONSENSUS_ERROR: ERRCODE: 3004 - " + _("Message transaction guid too big");
			return error(errorMessage.c_str());
		}
		if(theMessage.vchSubject.size() > MAX_NAME_LENGTH)
		{
			errorMessage = "PRESIDENTIELCOIN_MESSAGE_CONSENSUS_ERROR: ERRCODE: 3005 - " + _("Message subject too long");
			return error(errorMessage.c_str());
		}
		if(theMessage.vchMessageTo.size() > MAX_ENCRYPTED_MESSAGE_LENGTH)
		{
			errorMessage = "PRESIDENTIELCOIN_MESSAGE_CONSENSUS_ERROR: ERRCODE: 3006 - " + _("Message too long");
			return error(errorMessage.c_str());
		}
		if(theMessage.vchMessageFrom.size() > MAX_ENCRYPTED_VALUE_LENGTH)
		{
			errorMessage = "PRESIDENTIELCOIN_MESSAGE_CONSENSUS_ERROR: ERRCODE: 3007 - " + _("Message too long");
			return error(errorMessage.c_str());
		}
		if(!theMessage.vchMessage.empty() && theMessage.vchMessage != vvchArgs[0])
		{
			errorMessage = "PRESIDENTIELCOIN_MESSAGE_CONSENSUS_ERROR: ERRCODE: 3008 - " + _("Message guid in data output does not match guid in transaction");
			return error(errorMessage.c_str());
		}
		if(!IsValidAliasName(theMessage.vchAliasFrom))
		{
			errorMessage = "PRESIDENTIELCOIN_MESSAGE_CONSENSUS_ERROR: ERRCODE: 3009 - " + _("Alias name does not follow the domain name specification");
			return error(errorMessage.c_str());
		}
		if(!IsValidAliasName(theMessage.vchAliasTo))
		{
			errorMessage = "PRESIDENTIELCOIN_MESSAGE_CONSENSUS_ERROR: ERRCODE: 3010 - " + _("Alias name does not follow the domain name specification");
			return error(errorMessage.c_str());
		}
		if(op == OP_MESSAGE_ACTIVATE)
		{
			...
			{
				errorMessage = "PRESIDENTIELCOIN_MESSAGE_CONSENSUS_ERROR: ERRCODE: 3011 - " + _("Alias not provided as input");
				return error(errorMessage.c_str());
			}
			if (theMessage.vchMessage != vvchArgs[0])
			{
				errorMessage = "PRESIDENTIELCOIN_MESSAGE_CONSENSUS_ERROR: ERRCODE: 3012 - " + _("Message guid mismatch");
				return error(errorMessage.c_str());
			}

		}
		else{
			errorMessage = "PRESIDENTIELCOIN_MESSAGE_CONSENSUS_ERROR: ERRCODE: 3013 - " + _("Message transaction has unknown op");
			return error(errorMessage.c_str());
		}
	}
    ...
    if (!fJustCheck ) {
		...
		{
			errorMessage = "PRESIDENTIELCOIN_MESSAGE_CONSENSUS_ERROR: ERRCODE: 3014 - " + _("Cannot find alias for the recipient of this message. It may be expired");
			return true;
		}

		vector<CMessage> vtxPos;
		if (pmessagedb->ExistsMessage(vvchArgs[0])) {
			errorMessage = "PRESIDENTIELCOIN_MESSAGE_CONSENSUS_ERROR: ERRCODE: 3016 - " + _("This message already exists");
			return true;
		}      
		...
		{
			errorMessage = "PRESIDENTIELCOIN_MESSAGE_CONSENSUS_ERROR: ERRCODE: 3016 - " + _("Failed to write to message DB");
            return error(errorMessage.c_str());
		}
...
UniValue messagenew(const UniValue& params, bool fHelp) {
	...
	if (!GetTxOfAlias(vchFromString(strFromAddress), aliasFrom, aliastx))
		throw runtime_error("PRESIDENTIELCOIN_MESSAGE_RPC_ERROR: ERRCODE: 3500 - " + _("Could not find an alias with this name"));
    if(!IsMyAlias(aliasFrom)) {
		throw runtime_error("PRESIDENTIELCOIN_MESSAGE_RPC_ERROR: ERRCODE: 3501 - " + _("This alias is not yours"));
    }
	...
	if (wtxAliasIn == NULL)
	{
		...
		throw runtime_error("PRESIDENTIELCOIN_MESSAGE_RPC_ERROR: ERRCODE: 3502 - " + _("This alias is not in your wallet"));
	}
	...
	if(!GetTxOfAlias(vchFromString(strToAddress), aliasTo, aliastx))
	{
		...
		throw runtime_error("PRESIDENTIELCOIN_MESSAGE_RPC_ERROR: ERRCODE: 3503 - " + _("Failed to read to alias from alias DB"));
	}
	CPresidentielcoinAddress toAddr;
	GetAddress(aliasTo, &toAddr, scriptPubKeyOrig);


    // gather inputs
	vector<unsigned char> vchMessage = vchFromString(GeneratePresidentielcoinGuid());
	...
	if(!EncryptMessage(aliasTo, vchMessageByte, strCipherTextTo))
	{
		...
		throw runtime_error("PRESIDENTIELCOIN_MESSAGE_RPC_ERROR: ERRCODE: 3504 - " + _("Could not encrypt message data for receiver"));
	}
	...
	if(!EncryptMessage(aliasFrom, vchMessageByte, strCipherTextFrom))
	{
		...
		throw runtime_error("PRESIDENTIELCOIN_MESSAGE_RPC_ERROR: ERRCODE: 3505 - " + _("Could not encrypt message data for sender"));
	}
	...
	SendMoneyPresidentielcoin(vecSend, recipient.nAmount+fee.nAmount, false, wtx, wtxAliasIn, outPoint.n, aliasFrom.multiSigInfo.vchAliases.size() > 0);
...
UniValue messageinfo(const UniValue& params, bool fHelp) {
	...
	if (!pmessagedb->ReadMessage(vchMessage, vtxPos) || vtxPos.empty())
		throw runtime_error("PRESIDENTIELCOIN_MESSAGE_RPC_ERROR: ERRCODE: 3506 - " + _("Failed to read from message DB"));

	if(!BuildMessageJson(vtxPos.back(), oMessage))
		throw runtime_error("PRESIDENTIELCOIN_MESSAGE_RPC_ERROR: ERRCODE: 3507 - " + _("Could not find this message"));
...
UniValue messagereceivelist(const UniValue& params, bool fHelp) {
	...
	if(aliases.size() > 0)
	{
		if (!pmessagedb->ScanRecvMessages(vchNameUniq, aliases, 1000, messageScan))
			throw runtime_error("PRESIDENTIELCOIN_MESSAGE_RPC_ERROR: ERRCODE: 3508 - " + _("Scan failed"));
	}
	else
	{
		BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, pwalletMain->mapWallet)
		{
			...
			if (wtx.nVersion != PRESIDENTIELCOIN_TX_VERSION)
				continue;
			if(!IsPresidentielcoinTxMine(wtx, "message"))
				continue;
	...
	if(aliases.size() > 0)
	{
		for(unsigned int aliasIndex =0;aliasIndex<aliases.size();aliasIndex++)
		{
			...
			if (!paliasdb->ReadAlias(vchAlias, vtxPos) || vtxPos.empty())
				throw runtime_error("PRESIDENTIELCOIN_MESSAGE_RPC_ERROR: ERRCODE: 3509 - " + _("Failed to read from alias DB"));
			...
			if (!GetPresidentielcoinTransaction(alias.nHeight, alias.txHash, aliastx, Params().GetConsensus()))
				throw runtime_error("PRESIDENTIELCOIN_MESSAGE_RPC_ERROR: ERRCODE: 3510 - " + _("Failed to read alias transaction"));
