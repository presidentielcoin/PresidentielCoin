// Copyright (c) 2014 Presidentielcoin Developers
...
bool GetPresidentielcoinTransaction(int nHeight, const uint256 &hash, CTransaction &txOut, const Consensus::Params& consensusParams)
{
...
bool GetTimeToPrune(const CScript& scriptPubKey, uint64_t &nTime)
{
	...
	if(!GetPresidentielcoinData(scriptPubKey, vchData, vchHash))
		return false;
	if(!chainActive.Tip())
		return false;
	CAliasIndex alias;
...
bool IsPresidentielcoinScript(const CScript& scriptPubKey, int &op, vector<vector<unsigned char> > &vvchArgs)
{
...
void RemovePresidentielcoinScript(const CScript& scriptPubKeyIn, CScript& scriptPubKeyOut)
{
...
// how much is 1.1 BTC in presidentielcoin? 1 BTC = 110000 PRC for example, nPrice would be 1.1, sysPrice would be 110000
CAmount convertCurrencyCodeToPresidentielcoin(const vector<unsigned char> &vchAliasPeg, const vector<unsigned char> &vchCurrencyCode, const double &nPrice, const unsigned int &nHeight, int &precision)
{
	CAmount sysPrice = 0;
	double nRate = 1;
	...
	{
		if(fDebug)
			LogPrintf("convertCurrencyCodeToPresidentielcoin() Exception caught getting rate alias information\n");
	}
	if(precision > 8)
		sysPrice = 0;
	return sysPrice;
...
// convert 110000*COIN PRC into 1.1*COIN BTC
CAmount convertPresidentielcoinToCurrencyCode(const vector<unsigned char> &vchAliasPeg, const vector<unsigned char> &vchCurrencyCode, const CAmount &nPrice, const unsigned int &nHeight, int &precision)
{
	...
	{
		if(fDebug)
			LogPrintf("convertPresidentielcoinToCurrencyCode() Exception caught getting rate alias information\n");
	}
	if(precision > 8)
		currencyPrice = 0;
...
int GetPresidentielcoinDataOutput(const CTransaction& tx) {
   for(unsigned int i = 0; i<tx.vout.size();i++) {
	   if(IsPresidentielcoinDataOutput(tx.vout[i]))
		   return i;
	}
   return -1;
}
bool IsPresidentielcoinDataOutput(const CTxOut& out) {
...
int GetPresidentielcoinTxVersion()
{
	return PRESIDENTIELCOIN_TX_VERSION;
}

/**
 * [IsPresidentielcoinTxMine check if this transaction is mine or not, must contain a presidentielcoin service vout]
 * @param  tx [presidentielcoin based transaction]
 * @param  type [the type of presidentielcoin service you expect in this transaction]
 * @return    [if presidentielcoin transaction is yours based on type passed in]
 */
bool IsPresidentielcoinTxMine(const CTransaction& tx, const string &type) {
...
bool CheckAliasInputs(const CTransaction &tx, int op, int nOut, const vector<vector<unsigned char> > &vvchArgs, const CCoinsViewCache &inputs, bool fJustCheck, int nHeight, string &errorMessage, bool dontaddtodb) {
	...
	if (tx.nVersion != PRESIDENTIELCOIN_TX_VERSION) 
	{
		errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5000 - " + _("Non-Presidentielcoin transaction found");
		return true;
	}
	...
	{
		bool bData = GetPresidentielcoinData(tx, vchData, vchHash, nDataOut);
	...
	if(fJustCheck)
	{
		if(op != OP_ALIAS_PAYMENT)
		{
			if(vvchArgs.size() != 3)
			{
				errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5001 - " + _("Alias arguments incorrect size");
				return error(errorMessage.c_str());
			}
		}
		else if(vvchArgs.size() != 1)
		{
			errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5002 - " + _("Alias arguments incorrect size");
			return error(errorMessage.c_str());
		}
		if(op != OP_ALIAS_PAYMENT)
		{
			if(!theAlias.IsNull())
			{
				...
				{
					errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5003 - " + _("Hash provided doesn't match the calculated hash of the data");
					return true;
				}
			}					
		}
	...
	if(fJustCheck)
	{
		...
		{
			errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5004 - " + _("Alias name does not follow the domain name specification");
			return error(errorMessage.c_str());
		}
		...
		{
			errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5005 - " + _("Alias public value too big");
			return error(errorMessage.c_str());
		}
		if(theAlias.vchPrivateValue.size() > MAX_ENCRYPTED_VALUE_LENGTH)
		{
			errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5006 - " + _("Alias private value too big");
			return error(errorMessage.c_str());
		}
		if(theAlias.vchAliasPeg.size() > MAX_GUID_LENGTH)
		{
			errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5007 - " + _("Alias peg too long");
			return error(errorMessage.c_str());
		}
		if(theAlias.vchPassword.size() > MAX_ENCRYPTED_NAME_LENGTH)
		{
			errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5008 - " + _("Alias password too long");
			return error(errorMessage.c_str());
		}
		if(theAlias.nHeight > nHeight)
		{
			errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5009 - " + _("Bad alias height");
			return error(errorMessage.c_str());
		}
		...
		switch (op) {
			...
			case OP_ALIAS_ACTIVATE:
				...
				{
					errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5010 - " + _("Alias input guid mismatch");
					return error(errorMessage.c_str());
				}
				if(theAlias.vchAlias != vvchArgs[0])
				{
					errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5011 - " + _("Guid in data output doesn't match guid in tx");
					return error(errorMessage.c_str());
				}
				break;
			case OP_ALIAS_UPDATE:
				if (!IsAliasOp(prevOp))
				{
					errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5012 - " + _("Alias input to this transaction not found");
					return error(errorMessage.c_str());
				}
				// Check GUID
				if (vvchArgs.size() <= 1 || vvchPrevArgs.size() <= 1 || vvchPrevArgs[1] != vvchArgs[1])
				{
					errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5013 - " + _("Alias Guid input mismatch");
					return error(errorMessage.c_str());
				}
				// Check name
				if (vvchPrevArgs[0] != vvchArgs[0])
				{
					errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5014 - " + _("Alias input mismatch");
					return error(errorMessage.c_str());
				}
				if(!theAlias.IsNull())
				{
					if(theAlias.vchAlias != vvchArgs[0])
					{
						errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5015 - " + _("Guid in data output doesn't match guid in transaction");
						return error(errorMessage.c_str());
					}
				}
				break;
		default:
				errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5016 - " + _("Alias transaction has unknown op");
				return error(errorMessage.c_str());
		}
	...
	if (!fJustCheck ) {
		...
		if(!GetVtxOfAlias(vchAlias, dbAlias, vtxPos, isExpired))	
		{
			if(op == OP_ALIAS_ACTIVATE)
			{
				...
				{
					errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5017 - " + _("Trying to renew an alias that isn't expired");
					return true;
				}
			}
			else if(op == OP_ALIAS_UPDATE)
			{
				errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5018 - " + _("Failed to read from alias DB");
				return true;
			}
		...
		if(!vchData.empty())
		{
			...
			{
				errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5019 - " + _("Transaction does not pay enough fees");
				return true;
			}
		}
		...
		if(op == OP_ALIAS_UPDATE)
		{
			if(!vtxPos.empty())
			{
				...
				{
					errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5020 - " + _("Cannot extract destination of alias input");
					theAlias = dbAlias;
				}
				else
				{
					const CAliasIndex& destAlias = vtxPos.back();
					CPresidentielcoinAddress prevaddy(aliasDest);	
					CPresidentielcoinAddress destaddy;
					GetAddress(destAlias, &destaddy);
					if(destaddy.ToString() != prevaddy.ToString())
					{
						errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5021 - " + _("You are not the owner of this alias");
						theAlias = dbAlias;
					}
				}
				if(dbAlias.vchGUID != vvchArgs[1])
				{
					errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5022 - " + _("Cannot edit this alias, guid mismatch");
					theAlias = dbAlias;
				}
				if(theAlias.IsNull())
				...
				else
				{
					...
					if(!theAlias.multiSigInfo.IsNull())
					{
						...
						{
							errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5023 - " + _("Alias multisig too big, reduce the number of signatures required for this alias and try again");
						...
						{
							errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5024 - " + _("Cannot update multisig alias because required signatures is greator than the amount of signatures provided");
							theAlias.multiSigInfo.SetNull();
						}	
						...
						{
							errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5025 - " + _("Invalid redeem script provided in transaction");
							theAlias.multiSigInfo.SetNull();
						}
					}
					...
					if(dbAlias.vchPubKey != theAlias.vchPubKey)
					{
						...
						CPresidentielcoinAddress myAddress;
						...
						if (paliasdb->ExistsAddress(vchAddress))
						{
							...
							{
								errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5026 - " + _("An alias already exists with that address, try another public key");
								theAlias = dbAlias;
							}
						}					
					}
			...
			{
				errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5027 -" + _("Alias not found when trying to update");
				return true;
			}
		}
		else if(op == OP_ALIAS_ACTIVATE)
		{
			if(!isExpired && !vtxPos.empty())
			{
				errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5028 - " + _("Trying to renew an alias that isn't expired");
				return true;
			}
			...
			if(theAlias.multiSigInfo.vchAliases.size() > 0)
			{
				...
				{
					theAlias.multiSigInfo.SetNull();
					errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5029 - " + _("Alias multisig too big, reduce the number of signatures required for this alias and try again");
				}
				std::vector<CPubKey> pubkeys; 
				CPubKey pubkey(theAlias.vchPubKey);
				pubkeys.push_back(pubkey);
				...
				{
					theAlias.multiSigInfo.SetNull();
					errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5030 - " + _("Cannot update multisig alias because required signatures is greator than the amount of signatures provided");
				}
				CScript inner = GetScriptForMultisig(theAlias.multiSigInfo.nRequiredSigs, pubkeys);
				CScript redeemScript(theAlias.multiSigInfo.vchRedeemScript.begin(), theAlias.multiSigInfo.vchRedeemScript.end());
				if(redeemScript != inner)
				{
					theAlias.multiSigInfo.SetNull();
					errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5031 - " + _("Invalid redeem script provided in transaction");
				}
			}
		}
		else if(op == OP_ALIAS_PAYMENT)
		{
			...
			if(paliasdb->ExistsAliasPayment(vchAlias))
			{
				...
				{
					errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5032 - " + _("Cannot read payments from alias DB");
					return true;
				}
			}
			...
			{
				errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5034 - " + _("Failed to write payment to alias DB");
				return error(errorMessage.c_str());
			}
			if(fDebug)
				LogPrintf(
		...
		{
			errorMessage = "PRESIDENTIELCOIN_ALIAS_CONSENSUS_ERROR: ERRCODE: 5034 - " + _("Failed to write to alias DB");
			return error(errorMessage.c_str());
		}
...
bool GetPresidentielcoinData(const CTransaction &tx, vector<unsigned char> &vchData, vector<unsigned char> &vchHash, int& nOut)
{
	nOut = GetPresidentielcoinDataOutput(tx);
    if(nOut == -1)
	   return false;

	const CScript &scriptPubKey = tx.vout[nOut].scriptPubKey;
	return GetPresidentielcoinData(scriptPubKey, vchData, vchHash);
}
bool IsValidAliasName(const std::vector<unsigned char> &vchAlias)
{
	return (vchAlias.size() <= MAX_GUID_LENGTH && vchAlias.size() >= 3);
}
bool GetPresidentielcoinData(const CScript &scriptPubKey, vector<unsigned char> &vchData, vector<unsigned char> &vchHash)
{
...
void GetAddress(const CAliasIndex& alias, CPresidentielcoinAddress* address,const uint32_t nPaymentOption)
{
	if(!address)
		return;
	CPubKey aliasPubKey(alias.vchPubKey);
	CChainParams::AddressType myAddressType = PaymentOptionToAddressType(nPaymentOption);
	address[0] = CPresidentielcoinAddress(aliasPubKey.GetID(), myAddressType);
	if(alias.multiSigInfo.vchAliases.size() > 0)
	{
		...
		address[0] = CPresidentielcoinAddress(innerID, myAddressType);
	}
}
void GetAddress(const CAliasIndex& alias, CPresidentielcoinAddress* address,CScript& script,const uint32_t nPaymentOption)
{
	if(!address)
		return;
	CPubKey aliasPubKey(alias.vchPubKey);
...
void CleanupPresidentielcoinServiceDatabases(int &numServicesCleaned)
{
	if(pofferdb != NULL)
...
bool GetAliasOfTx(const CTransaction& tx, vector<unsigned char>& name) {
	...
	if (!good)
		return error("GetAliasOfTx() : could not decode a presidentielcoin tx");
...
bool DecodeAndParsePresidentielcoinTx(const CTransaction& tx, int& op, int& nOut,
		vector<vector<unsigned char> >& vvch)
{
...
void CreateFeeRecipient(CScript& scriptPubKey, const vector<unsigned char>& vchAliasPeg, const uint64_t& nHeight, const vector<unsigned char>& data, CRecipient& recipient)
{
	...
	size_t nSize = txout.GetSerializeSize(SER_DISK,0)+148u;
	int nFeePerByte = getFeePerByte(vchAliasPeg, vchFromString("PRC"), nHeight, precision);
	if(nFeePerByte <= 0)
...
UniValue aliasauthenticate(const UniValue& params, bool fHelp) {
	...
	if (!GetTxOfAlias(vchAlias, theAlias, tx, true))
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5500 - " + _("Could not find an alias with this name"));
	...
	if(strPassword.empty())
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5501 - " + _("Password cannot be empty"));

    if(!crypt.SetKeyFromPassphrase(strPassword, vchAliasHash, 25000, 0))
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5502 - " + _("Could not determine key from password"));

	CKey key;
	key.Set(crypt.chKey, crypt.chKey + (sizeof crypt.chKey), true);
	CPubKey defaultKey = key.GetPubKey();
	if(!defaultKey.IsFullyValid())
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5503 - " + _("Generated public key not fully valid"));

	if(aliasPubKey != defaultKey)
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5504 - " + _("Password is incorrect"));
	UniValue res(UniValue::VOBJ);
	res.push_back(Pair("privatekey", CPresidentielcoinSecret(key).ToString()));
	return res;
...
void TransferAliasBalances(const vector<unsigned char> &vchAlias, const CScript& scriptPubKeyTo, vector<CRecipient> &vecSend, CCoinControl& coinControl){
	...
	CPresidentielcoinAddress addressFrom;
...
UniValue aliasnew(const UniValue& params, bool fHelp) {
	if (fHelp || 4 > params.size() || 10 < params.size())
		...
		"aliasnew <aliaspeg> <aliasname> <password> <public value> [private value] [safe search=Yes] [accept transfers=Yes] [expire=31536000] [nrequired=0] [\"alias\",...]\n"
						...
						"     [\n"
						"       \"alias\"    Existing presidentielcoin alias name\n"
						"       ,...\n"
						"     ]\n"						
	...
	The domain name should not start or end with hyphen (-) (e.g. -presidentielcoin.org or presidentielcoin-.org)
	...
	if(find_first(strName, "."))
	{
		if (!regex_search(strName, nameparts, domainwithtldregex) || string(nameparts[0]) != strName)
			throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5505 - " + _("Invalid Presidentielcoin Identity. Must follow the domain name spec of 3 to 64 characters with no preceding or trailing dashes and a TLD of 2 to 6 characters"));	
	}
	else
	{
		if (!regex_search(strName, nameparts, domainwithouttldregex)  || string(nameparts[0]) != strName)
			throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5506 - " + _("Invalid Presidentielcoin Identity. Must follow the domain name spec of 3 to 64 characters with no preceding or trailing dashes"));
	}
	...
	if(strPassword.size() < 4 && strPassword.size() > 0)
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5507 - " + _("Invalid Presidentielcoin Identity. Please enter a password atleast 4 characters long"));
	...
	if(aliasExists && !isExpired)
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5508 - " + _("This alias already exists"));
	...
	CPresidentielcoinAddress oldAddress(defaultKey.GetID());
	if(!strPassword.empty())
	{
		CCrypter crypt;
		uint256 hashAliasNum = Hash(vchAlias.begin(), vchAlias.end());
		...
		if(!crypt.SetKeyFromPassphrase(password, vchAliasHash, 25000, 0))
			throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5509 - " + _("Could not determine key from password"));
		CKey key;
		key.Set(crypt.chKey, crypt.chKey + (sizeof crypt.chKey), true);
		defaultKey = key.GetPubKey();
		if(!defaultKey.IsFullyValid())
			throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5510 - " + _("Generated public key not fully valid"));
		CKey keyTmp;
		if(!pwalletMain->GetKey(defaultKey.GetID(), keyTmp) && !pwalletMain->AddKeyPubKey(key, defaultKey))	
			throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5511 - " + _("Please choose a different password"));
	}
	...
	if(aliasNames.size() > 0)
	{
		...
		for(int i =0;i<aliasNames.size();i++)
		{
			...
			if (!GetTxOfAlias( vchFromString(aliasNames[i].get_str()), multiSigAlias, txMultiSigAlias))
				throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5512 - " + _("Could not find multisig alias with the name: ") + aliasNames[i].get_str());
			...
			if(!EncryptMessage(vchMSPubKey, vchEncryptionPrivateKey, strCipherText))
				throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5513 - " + _("Could not encrypt private encryption key!"));
	...
	CPresidentielcoinAddress newAddress = CPresidentielcoinAddress(CScriptID(scriptPubKeyOrig));	
	...
	{
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5514 - " + _("Could not encrypt private alias value!"));
	}
	vchPrivateValue = vchFromString(strCipherText);

	if(!EncryptMessage(vchPubKey, vchEncryptionPrivateKey, strCipherText))
	{
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5515 - " + _("Could not encrypt private encryption key!"));
	}
	...
	if(!strPassword.empty())
	{
		...
		{
			throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5516 - " + _("Could not encrypt alias password"));
		}
		strPassword = strCipherText;
	}
	vector<unsigned char> vchRandAlias = vchFromString(GeneratePresidentielcoinGuid());
	...
	SendMoneyPresidentielcoin(vecSend, recipient.nAmount + fee.nAmount, false, wtx, NULL, 0, oldAlias.multiSigInfo.vchAliases.size() > 0, coinControl.HasSelected()? &coinControl: NULL);
...
UniValue aliasupdate(const UniValue& params, bool fHelp) {
	if (fHelp || 3 > params.size() || 11 < params.size())
		...
		"aliasupdate <aliaspeg> <aliasname> <public value> [private value=''] [safesearch=Yes] [toalias_pubkey=''] [password=''] [accept transfers=Yes] [expire=31536000] [nrequired=0] [\"alias\",...]\n"
						...
						"<toalias_pubkey> receiver presidentielcoin alias pub key, if transferring alias.\n"
						"<accept transfers> set to No if this alias should not allow a certificate to be transferred to it. Defaults to Yes.\n"		
	...
	if(strPassword.size() < 4 && strPassword.size() > 0)
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5517 - " + _("Invalid Presidentielcoin Identity. Please enter a password atleast 4 characters long"));
	...
	CAliasIndex theAlias;
	if (!GetTxOfAlias(vchAlias, theAlias, tx, true))
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5518 - " + _("Could not find an alias with this name"));

	COutPoint outPoint;
	int numResults  = aliasunspent(vchAlias, outPoint);
	const CWalletTx* wtxIn = pwalletMain->GetWalletTx(outPoint.hash);
	if (wtxIn == NULL)
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5519 - " + _("This alias is not in your wallet"));

	CPresidentielcoinAddress oldAddress;
	...
	if(!strPassword.empty())
	{
		...
		SecureString password = pwStr.c_str();
		if(!crypt.SetKeyFromPassphrase(password, vchAliasHash, 25000, 0))
			throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5520 - " + _("Could not determine key from password"));
		CKey key;
		key.Set(crypt.chKey, crypt.chKey + (sizeof crypt.chKey), true);
		pubKey = key.GetPubKey();
		
		if(!pubKey.IsFullyValid())
			throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5521 - " + _("Generated public key not fully valid"));	
		CKey keyTmp;
		if(!pwalletMain->GetKey(pubKey.GetID(), keyTmp) && !pwalletMain->AddKeyPubKey(key, pubKey))	
			throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5522 - " + _("Please choose a different password"));	
	}
	...
	{
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5523 - " + _("Could not decrypt alias encryption private key"));
	}
	...
	if(!vchPrivateValue.empty())
	{
		...
		if(!copyAlias.vchPrivateValue.empty() && !transferAlias)
		{
			...
			{
				throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5524 - " + _("Could not decrypt alias private data"));
			}
			...
			else
			{
				if(!EncryptMessage(vchEncryptionPublicKey, vchPrivateValue, strCipherText))
				{
					throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5525 - " + _("Could not encrypt alias private data"));
				}
				vchPrivateValue = vchFromString(strCipherText);
			}
		}
		else
		{
			if(!EncryptMessage(vchEncryptionPublicKey, vchPrivateValue, strCipherText))
			{
				throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5526 - " + _("Could not encrypt alias private data"));
			}
			vchPrivateValue = vchFromString(strCipherText);
		}
	}
	if(!strPassword.empty())
	{
		...
		{
			throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5527 - " + _("Could not encrypt alias password"));
		}
		strPassword = strCipherText;
	}
	CMultiSigAliasInfo multiSigInfo;
	if(aliasNames.size() > 0)
	{
		...
		for(int i =0;i<aliasNames.size();i++)
		{
			...
			CTransaction txMultiSigAlias;
			if (!GetTxOfAlias( vchFromString(aliasNames[i].get_str()), multiSigAlias, txMultiSigAlias))
				throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5528 - " + _("Could not find multisig alias with the name: ") + aliasNames[i].get_str());
			...
			vector<unsigned char> vchMSPubKey(pubkey.begin(), pubkey.end());
			if(!EncryptMessage(vchMSPubKey, vchEncryptionPrivateKey, strCipherText))
				throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5529 - " + _("Could not encrypt private encryption key!"));
			multiSigInfo.vchEncryptionPrivateKeys.push_back(strCipherText);
		}	
	...
	{
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5530 - " + _("Could not encrypt private encryption key!"));
	}
	vchEncryptionPrivateKey = vchFromString(strCipherText);
	...
	CPresidentielcoinAddress newAddress;
	CScript scriptPubKeyOrig;
	...
	SendMoneyPresidentielcoin(vecSend, recipient.nAmount+fee.nAmount, false, wtx, wtxIn, outPoint.n, copyAlias.multiSigInfo.vchAliases.size() > 0, coinControl.HasSelected()? &coinControl: NULL);
	UniValue res(UniValue::VARR);
...
UniValue presidentielcoindecoderawtransaction(const UniValue& params, bool fHelp) {
	if (fHelp || 1 != params.size())
		throw runtime_error("presidentielcoindecoderawtransaction <alias> <hexstring>\n"
		"Decode raw presidentielcoin transaction (serialized, hex-encoded) and display information pertaining to the service that is included in the transactiion data output(OP_RETURN)\n"
	...
	if(rawTx.IsNull())
	{
		throw runtime_error("PRESIDENTIELCOIN_RPC_ERROR: ERRCODE: 5531 - " + _("Could not decode transaction"));
	}
	vector<unsigned char> vchData;
	int nOut;
	int op;
	vector<vector<unsigned char> > vvch;
	vector<unsigned char> vchHash;
	GetPresidentielcoinData(rawTx, vchData, vchHash, nOut);	
	UniValue output(UniValue::VOBJ);
	if(DecodeAndParsePresidentielcoinTx(rawTx, op, nOut, vvch))
	...
	for (unsigned int i = 0; i < rawTx.vout.size(); i++) {
		...
		if(!IsPresidentielcoinDataOutput(rawTx.vout[i]) && (!IsPresidentielcoinScript(rawTx.vout[i].scriptPubKey, tmpOp, tmpvvch) || tmpOp == OP_ALIAS_PAYMENT))
		{
...
void AliasTxToJSON(const int op, const vector<unsigned char> &vchData, const vector<unsigned char> &vchHash, UniValue &entry)
{
	...
	CPresidentielcoinAddress address;
	GetAddress(alias, &address);
	CPresidentielcoinAddress dbaddress;
...
UniValue presidentielcoinsignrawtransaction(const UniValue& params, bool fHelp) {
	if (fHelp || 1 != params.size())
		throw runtime_error("presidentielcoinsignrawtransaction <hexstring>\n"
	...
	{
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5532 - " + _("Could not sign multisig transaction: ") + find_value(objError, "message").get_str());
	}	
	if (!res.isObject())
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5533 - " + _("Could not sign multisig transaction: Invalid response from signrawtransaction"));
	...
	if(bComplete)
	{
		...
		if (!returnRes.isStr())
			throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5534 - " + _("Could not send raw transaction: Invalid response from sendrawtransaction"));
	}
	return res;
}
bool IsMyAlias(const CAliasIndex& alias)
{
	...
	CPubKey aliasPubKey(alias.vchPubKey);
	CPresidentielcoinAddress address(aliasPubKey.GetID());
	if(alias.multiSigInfo.vchAliases.size() > 0)
	{
		CScript inner(alias.multiSigInfo.vchRedeemScript.begin(), alias.multiSigInfo.vchRedeemScript.end());
...
UniValue aliaslist(const UniValue& params, bool fHelp) {
	...
	BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, pwalletMain->mapWallet) {
		...
		// skip non-presidentielcoin txns
		if (wtx.nVersion != PRESIDENTIELCOIN_TX_VERSION)
			continue;
		...
		{
			pending = 1;
			if(!IsPresidentielcoinTxMine(wtx, "alias"))
				continue;
		}
		else
		{
			alias = vtxPos.back();
			CTransaction tx;
			if (!GetPresidentielcoinTransaction(alias.nHeight, alias.txHash, tx, Params().GetConsensus()))
			{
				pending = 1;
				if(!IsPresidentielcoinTxMine(wtx, "alias"))
					continue;
	...
	map<vector<unsigned char>, UniValue> vOfferO;
	{
		...
		BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, pwalletMain->mapWallet) {
            ...
            if (!DecodeOfferTx(wtx, op, nOut, vvch) 
            	...
            	|| (op == OP_OFFER_ACCEPT))
                continue;
			if(!IsPresidentielcoinTxMine(wtx, "offer"))
					continue;
            vchOffer = vvch[0];
...
string GeneratePresidentielcoinGuid()
{
...
UniValue aliasbalance(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"alias\"  (string, required) The presidentielcoin alias for transactions.\n"
...
int aliasunspent(const vector<unsigned char> &vchAlias, COutPoint& outpoint)
{
	...
	CTxDestination aliasDest;
	CPresidentielcoinAddress prevaddy;
	int numResults = 0;
	CCoinsViewCache view(pcoinsTip);
	const CCoins *coins;
	bool found = false;
		...
		if(coins == NULL)
         ...
         for (unsigned int j = 0;j<coins->vout.size();j++)
		 {
			...
			if (!ExtractDestination(coins->vout[j].scriptPubKey, aliasDest))
				continue;
			prevaddy = CPresidentielcoinAddress(aliasDest);
			if(destaddy.ToString() != prevaddy.ToString())
				continue;
...
UniValue aliasinfo(const UniValue& params, bool fHelp) {
	...
	vector<CAliasIndex> vtxPos;
	if (!paliasdb->ReadAlias(vchAlias, vtxPos) || vtxPos.empty())
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5535 - " + _("Failed to read from alias DB"));

	UniValue oName(UniValue::VOBJ);
	if(!BuildAliasJson(vtxPos.back(), 0, oName, strPrivateKey))
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5536 - " + _("Could not find this alias"));
...
UniValue aliashistory(const UniValue& params, bool fHelp) {
	...
	vector<CAliasIndex> vtxPos;
	if (!paliasdb->ReadAlias(vchAlias, vtxPos) || vtxPos.empty())
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5537 - " + _("Failed to read from alias DB"));
...
UniValue aliasfilter(const UniValue& params, bool fHelp) {
	...
	if (!paliasdb->ScanNames(vchAlias, strRegexp, safeSearch, 25, nameScan))
		throw runtime_error("PRESIDENTIELCOIN_ALIAS_RPC_ERROR: ERRCODE: 5538 - " + _("Scan failed"));
...
void GetPrivateKeysFromScript(const CScript& script, vector<string> &strKeys)
{
	...
	BOOST_FOREACH(const CTxDestination& txDest, addrs) {
		CPresidentielcoinAddress address(txDest);
		CKeyID keyID;
		...
		if (!pwalletMain->GetKey(keyID, vchSecret))
			continue;
		strKeys.push_back(CPresidentielcoinSecret(vchSecret).ToString());
	}
}