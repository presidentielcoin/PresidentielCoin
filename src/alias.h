...
class CCoins;
class CBlock;
class CPresidentielcoinAddress;
class COutPoint;
struct CRecipient;
...
const int PRESIDENTIELCOIN_TX_VERSION = 0x7400;
...
bool IsPresidentielcoinTxMine(const CTransaction& tx,const std::string &type);
...
bool DecodeAndParsePresidentielcoinTx(const CTransaction& tx, int& op, int& nOut, std::vector<std::vector<unsigned char> >& vvch);
...
CAmount convertCurrencyCodeToPresidentielcoin(const std::vector<unsigned char> &vchAliasPeg, const std::vector<unsigned char> &vchCurrencyCode, const double &nPrice, const unsigned int &nHeight, int &precision);
...
CAmount convertPresidentielcoinToCurrencyCode(const std::vector<unsigned char> &vchAliasPeg, const std::vector<unsigned char> &vchCurrencyCode, const CAmount &nPrice, const unsigned int &nHeight, int &precision);
...
std::string GeneratePresidentielcoinGuid();
bool IsAliasOp(int op);
...
bool GetPresidentielcoinData(const CTransaction &tx, std::vector<unsigned char> &vchData, std::vector<unsigned char> &vchHash, int& nOut);
bool GetPresidentielcoinData(const CScript &scriptPubKey, std::vector<unsigned char> &vchData, std::vector<unsigned char> &vchHash);
...
bool IsPresidentielcoinScript(const CScript& scriptPubKey, int &op, std::vector<std::vector<unsigned char> > &vvchArgs);
...
void CleanupPresidentielcoinServiceDatabases(int &servicesCleaned);
int aliasunspent(const std::vector<unsigned char> &vchAlias, COutPoint& outpoint);
bool IsMyAlias(const CAliasIndex& alias);
void GetAddress(const CAliasIndex &alias, CPresidentielcoinAddress* address, const uint32_t nPaymentOption=1);
void GetAddress(const CAliasIndex &alias, CPresidentielcoinAddress* address, CScript& script, const uint32_t nPaymentOption=1);
