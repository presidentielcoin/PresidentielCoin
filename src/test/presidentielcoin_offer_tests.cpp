...
BOOST_GLOBAL_FIXTURE( PresidentielcoinTestingSetup );

BOOST_FIXTURE_TEST_SUITE (presidentielcoin_offer_tests, BasicPresidentielcoinTestingSetup)
...
BOOST_AUTO_TEST_CASE (generate_certoffer)
{
	...
	// generate a cert offer if accepting BTC OR PRC
	OfferNew("node1", "node1alias", "certificates", "title", "1", "0.05", "description", "USD", certguid1, "PRC+BTC");
...
BOOST_AUTO_TEST_CASE (generate_offerwhitelists)
{
	...
	// generate a good offer
	string offerguid = OfferNew("node1", "sellerwhitelistalias", "category", "title", "100", "10.00", "description", "PRC", "nocert");
	// add to whitelist
...
BOOST_AUTO_TEST_CASE (generate_offerupdate_editcurrency)
{
	...
	OfferUpdate("node1", "selleraliascurrency", offerguid, "category", "titlenew", "90", "1.00", "descriptionnew", "PRC");
