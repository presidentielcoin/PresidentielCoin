#ifndef PRESIDENTIELCOIN_TEST_TEST_PRESIDENTIELCOIN_SERVICES_H
#define PRESIDENTIELCOIN_TEST_TEST_PRESIDENTIELCOIN_SERVICES_H

#include <stdio.h>
#include <univalue.h>
using namespace std;
/** Testing presidentielcoin services setup that configures a complete environment with 3 nodes.
 */
...
struct PresidentielcoinTestingSetup {
    PresidentielcoinTestingSetup();
    ~PresidentielcoinTestingSetup();
};
struct BasicPresidentielcoinTestingSetup {
    BasicPresidentielcoinTestingSetup();
    ~BasicPresidentielcoinTestingSetup();
};
struct PresidentielcoinMainNetSetup {
	PresidentielcoinMainNetSetup();
	~PresidentielcoinMainNetSetup();
};
#endif
