Presidentielcoin Core version 0.11.0 is now available from:

  <https://presidentielcoin.org/bin/presidentielcoin-core-0.11.0/>
...
For example, add the following to `presidentielcoin.conf`:
...
related to the blockchain in the presidentielcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Presidentielcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Presidentielcoin Core will 
...
For an example script see [presidentielcoin-submittx](https://github.com/laanwj/presidentielcoin-submittx).
...
- #6269 `95aca44` gitian: Use the new presidentielcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only PresidentielcoinAmountField
- #5753 `0f44672` Add presidentielcoin logo to about screen
...
- #5809 `46bfbe7` Add presidentielcoin-cli man page
...
- #5849 `41259ca` contrib/init/presidentielcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Presidentielcoin-Qt.app to Presidentielcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for presidentielcoin-tx)
...
- Ben Holden-Crowther
- bikinibabe
- PresidentielcoinPRReadingGroup
- Blake Jakopovic
- BtcDrak
- charlescharles
- Chris Arnesen
